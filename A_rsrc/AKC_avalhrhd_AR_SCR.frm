TYPE(drgForm) DTYPE(10) TITLE(Pøehled akcionáøù/ valných hromad ...) FILE(AKCIONAR)         ;
                                                                     SIZE(107,25)           ;
...
* AVALHRIT /zkrácení akcionáøi/ + vazba na AVALHRHD
  TYPE(TabPage) CAPTION( valné hromady) FPOS(0, 14.2) SIZE(107,10.5) RESIZE(yx) TTYPE(3) OFFSET(1,81)

    TYPE(DBrowse) FILE(AVALHRIT) INDEXORD(1)                       ;
                  FIELDS( nporVALhro:poøVh                       , ;
                          avalhrhd->ctypVALhro:typVh             , ;   
                          avalhrhd->ddatKonani:datKonání:10      , ;
                          avalhrhd->ccasKonani:èasKonání         , ;
                          avalhrhd->cjmenoJedn:jméno jednatele:35, ;
                          avalhrhd->cmisto:místo:30              , ;
                          avalhrhd->npocetAkci:poètAkcií:10      , ;
                          avalhrhd->nhodnotaAk:hodnota:13        , ;
                          avalhrhd->npocetHlas:poèHlasù:10         ) ;                         
                 SIZE(107, 9.6) RESIZE(yx) CURSORMODE(3) PP(7) SCROLL(yy)
  TYPE(End)

* AKCIONAR - údaje o akcionáøi
  TYPE(TabPage) CAPTION( info Akcionáøe) FPOS(0, 14.2) SIZE(107,10.5) RESIZE(yx) TTYPE(3) OFFSET(18,63)
