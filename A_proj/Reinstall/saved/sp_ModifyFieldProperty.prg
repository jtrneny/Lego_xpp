...
static pa_system := { ;
 { 's_tables' , "select left(Name,20)              as Name,"           + ;
                       "Table_Encryption           as Encryption from system.tables"    }, ;
 { 's_columns', "select left(Name  ,20)            as Name  ,"         + ;
                       "left(Parent,20)            as Parent,"         + ;
                       "Field_Num,"                                    + ;
                       "Field_Length,"                                 + ;
...
*  for x := 1 to len(pa_system) step 1
    ...
    *
    ...
    Ads_convertTable( hCursor, , cwork_File )
    DbUseArea( .t., 'FOXCDX', cwork_File, , .f.)

    cindexKey := pa_tags[x]
    bindexKey := &('{ || ' + cindexKey + '}')
    OrdCreate( cwork_File, cout_alias, cindexKey, bindexKey )
