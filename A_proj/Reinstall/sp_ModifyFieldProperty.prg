...
*  0
** ovìøíme, jestli k pøíslušní databázi jsou pøipojeni uživatelé
function get_table_users( tableUsers )
  local  cStatement, oStatement
  local  nusers := 99

  cStatement := "execute procedure sp_mgGetTableUsers( '" +tableUsers + "')"
...
*      setup_work_textinfo(oXbp_text, o_drgFile:description )
      ...
      * vybudujeme pomocnou strukturu pro kontrolu/ založení indexù
      ...
      paindex_Statement := check_dbd_indexes(fileName, aext_adi_struct, lnewTable )
...
**
      drgDump( fileName + ' pøed modifikací ')
...
**
      ...
      if oStatement:LastError > 0
        ...
        * --- asi je toho víc podle ERR
        ...
        oStatement:Close()
**
        drgDump( fileName + ' po modifikací')
**
      endif
...
static function create_ext_data_struct(o_drgFile)
  ...
  *
  ...
  for x := 1 to len(adesc) step 1
    ...
    adt_len  := if( lower(adt_type) = 's', 4, pao:adt_len )
    adt_dec  := pao:adt_dec
    *
...
static function alter_table(fileName, aext_data_struct)
  ...
  *
  local add_struct     := {}
  local add_del_column := {}, add_alter_column := {}, add_only_column := {}
  *
  local cDEFAULT
  local cStatement     := ''
  ...
  * musím modifikovat tabulku ?
  if len(add_alter_column) <> 0 .or. len(add_del_column) <> 0
    ...
    * ALTER
    if len(add_alter_column) <> 0
       ...
       for x := 1 to len(add_alter_column) step 1
         ...
         if pao.add_alter = 1
           if pao.dbd_Field_Num > len( add_struct)
             AAdd( add_only_column, pao )
    ...
    * ONLY ADD COLUMN
    if len( add_only_column) <> 0
      ASort( add_only_column, , , { |aX,aY| aX[9] > aY[9] } )

      for x := 1 to len(add_only_column) step 1
         pao        := add_only_column[x]
         cDefault   := if( isNull( pao.add_default), '', " DEFAULT " +pao.add_default )
...
static function check_dbd_indexes(fileName, aext_adi_struct, lnewTable)
  ...
  *
  local atagNames  := {}, phConnect := licasysw->phConnect
  ...
  *
  ** došlo k  porušení vazeb mezi DD a ADI, DD má v seznamu víc TAGU, než ADI
  ** musíme zrušit ADI a kompletnì pøebudovat
  if .not. lnewTable
    drgDBMS:open( fileName, .t., ,,, 'aliasData' )
    atagNames := aliasData->(ordList())
    aliasData->(dbcloseArea())

    if len(atagNames) <> len(add_struct)
      AdsDDRemoveIndexFile( phConnect, fileName, fileName +'.adi' )
      add_struct := {}
    endif
  endif
    ...
    otherwise
      ...
      case pao.adi_expression <> add_struct[npos,3] .or. ;
        ...
        * DROP and sp_CreateIndex
        cdrop_Index    += "DROP INDEX " +pao.adi_fileName +"." +pao.adi_tagName +";" +CRLF
        sp_CreateIndex := .t.
      endcase
    endcase
