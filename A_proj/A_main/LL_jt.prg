#include "Xbp.ch"
#include "dll.ch"
#include "..\A_main\cmbtll19.ch"
#include "..\A_main\cmbtls19.ch"
#include "common.ch"
#include "directry.ch"
#include "class.ch"
*
...
PROCEDURE LL_DefineDesign(isPrint,isdesc, isczech, cond, auto, poini)
  ...
  if isczech
    hJob := LlJobOpen(CMBTLANG_CZECH)
  else
    hJob := LlJobOpen(CMBTLANG_ENGLISH)
  endif

//  LlSetOptionString (hJob, LL_OPTIONSTR_LICENSINGINFO, "DfATOw")
      ...
      * jedná se o relaèní soubor a má jiný alias
      ...
      if pb[1] = 'FCE'
        if( npos := ascan( pa_Function, { |x| lower(x[3]) = lower(pb[2]) })) <> 0
          AAdd(adata, { pa_Function[npos,2],pa_Function[npos,1],pa_Function[npos,3]})
        endif
      endif
...
static procedure Print_DefineData(adata, isVariable)
  ...
  for x := 1 to len(adata) step 1
    ...
    if isVariable
      LlDefineVariableExt(hJob, adata[x].fields, FldContent, FldType, 0 )
    else
      LlDefineFieldExt(hJob, adata[x].fields, FldContent, FldType, 0 )
    endif
  next
return
...
STATIC PROCEDURE DefineData(isVariable, filem, filer, inDesign, isdesc, cdbd_File)
  ...
  *
  ...
  FOR i := 1 to fldCount STEP 1
    ...
    if isVariable
      LlDefineVariableExt(hJob, cvarName +if( empty(field_desc), '', ':') +field_desc, FldContent, FldType, 0 )

    elseif .not. isVariable
      LlDefineFieldExt(hJob, cvarName +if( empty(field_desc), '', ':') +field_desc, FldContent, FldType, 0 )
    endif
  NEXT I
RETURN


STATIC PROCEDURE DefineFce(isVariable, bfce, filer, ctype, caption)
  LOCAL  FldType, FldContent, DateBuffer, lExpr
  *
  ...
  *
  ...
  DO WHILE .not. (filema)->(Eof())
    ...
    if ValType( cfgValue) = 'A'
      cfgValue := Str( cfgValue[1]) + ':' +Str( cfgValue[2])
      cfgType  := 'C'
    endif
      ...
      for x := 1 to len(pa) step 1
        ...
        * jedná se o relaèní soubor a má jiný alias
        ...
        if IsObject(odbd := drgDBMS:dbd:getByKey(file))
          ...
          mFORMS_ll  := StrTran(mFORMS_ll,pa[x,1], field_temp +if( empty(field_desc), '', ':') +field_desc)
        endif

        if pa[x,2] = 'FCE'
//          if( npos := ascan( pa_Function, { |x| lower(x[3]) = lower(pa[x,3]) })) <> 0
          for n:=1 to len( pa_Function)
            if pa_Function[n,3] =  field_name
              field_desc := ''
              field_desc := if( isNull(field_desc),'', field_desc_mod(field_desc))
...
static function verify_by_UsedIdentifiers( inReadDesign )
  ...
  * je popiska ?
  for x := 1 to len(pa_inSections) step 1
    if at( ':', pa_inSections[x] ) <> 0
      pc  := listAsArray( pa_inSections[x], ':' )
      AAdd( pb, pc[1] )
    else
...
METHOD LLIniFile:SortOrder(buffer)
  ...
  *
  ...
  LOCAL  oldEXACT
  *
  Local  npos, cc, a_TagNames, cdirW
  ...
  if( asc(buffer) <> 0 .and. (n := at(chr(0), buffer)) > 0 )
    ...
    *
    ...
    case(tagNo =  0 .and. .not. empty(indexKey))
      ...
      ::cWorkCdx_LL  := drgINI:dir_USERfitm +userWorkDir() +'\Tisky'
...
*/
      ...
      DefineFce(.f., pa[1], pa[3], pa[2], pa[4] )
      AAdd( pa_Function, { pa[1], pa[2], pa[3] } )
