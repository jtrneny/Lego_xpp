...
#ifdef __LL24__
  #include "..\A_main\cmbtLL24.ch"
  #include "..\A_main\cmbtLS24.ch"
#else
...
STATIC  sName, sNameExt, hJob, cresetKey, xresetKey, llProjType // , mainTable
STATIC  pa_inSections, pa_Function
STATIC  sn_cmLL11 := 0, sn_cmPR11 := 0, sn_cmBR11 := 0, sn_cmCT11 := 0

* pro subTable
STATIC  pa_subTables, is_subTables, root_Table

* pro mì ardesáø pro pomocné tøídìní
static  is_SERVER01SW
...
PROCEDURE LL_DefineDesign(isPrint,isdesc, isczech, cond, auto, poini)
  ...
  ** pro mì ardesáø pro pomocné tøídìní
  ...
  if isczech
    hJob := LlJobOpen(CMBTLANG_CZECH)  // CMBTLANG_CZECH)
//    hJob := LlJobOpen(CMBTLANG_DEFAULT)  // CMBTLANG_CZECH)
  ...
  #ifdef __LL24__
    LlSetOptionString(hJob, LL_OPTIONSTR_LICENSINGINFO, "EEt0HQ")
  #else
...
PROCEDURE LL_PrintDesign(isdesc, typ, cond, auto, cPrinter, nCopies)
  ...
  local  variables, fields, pa, x, pb, file, field_name, adata := {}
  *
  local  aRet, bRet, nRetx
  local  lenBuff := 40960, buffer := space(lenBuff)
  local  print, txt, oini, pa_Rel := {}, npos, odbd, odrgrf
  *
  ...
  * první case je pro subTables
  *
  do case
  case ( llProjType = LL_PROJECT_LIST .and. is_subTables .and. .not. empty(root_Table) )
    nRet      := LL_WRN_TABLECHANGE
    rootCount := LlPrintDbGetRootTableCount(oini:hJob)

    if rootCount <> 0
      do while nRet == LL_WRN_TABLECHANGE
        nBuffer := LlPrintDbGetCurrentTable(oini:hJob, "",0,.T.)
        if nBuffer >0
        ...
        nRet := PrintLL_subTable(sCurrentTable, hJob, adata)
      enddo
    endif
...
static function gate(isL)
  ...
  next
return strtran(cC, ' ', '') +if( empty(cc), '', ' ' )


STATIC PROCEDURE DefineAsys()
  LOCAL  ll_cond := ''
  local  recNo
  local  clGate, cvyraz_1u, crelace, cvyraz_2u, crGate, coperand
  ...
  if select('filtritw') <> 0
    ...
    do while .not.filtritw ->(eof())
      clGate    := gate(.t.)
      cvyraz_1u := alltrim(filtritw ->cvyraz_1u)
      crelace   := alltrim(filtritw ->crelace  )
      cvyraz_2u := alltrim(filtritw ->cvyraz_2u)

      if empty(cvyraz_2u) .and. filtritW->ctype_1 = 'C'
        cvyraz_2u := '"' +space(filtritW->nlen_1) +'"'
      endif

      crGate    := gate(.f.)
      coperand  := if( empty( crGate), allTrim(filtritw ->coperand), crGate +allTrim(filtritw ->coperand) )
...
*
** pro zpracování subTable
**
static function PrintLL_subTable(sCurrentTable, hJob, adata)
  ...
  local cc, pa_arg := {}
   ...
   if sCurrentTable == "LLStaticTable"
     ...
     do case
     case (sCurrentTable = root_Table)
       dbSelectArea(root_Table)

     case( npos := ascan(pa_subTables, {|x| x[1] = sCurrentTable })) <> 0
       c_subTable := pa_subTables[npos,1]
       c_aof      := pa_subTables[npos,4]
       pa_forAof  := pa_subTables[npos,5]

       dbSelectArea(c_subTable)

       ** check whether is root table
       ...
       if IsNotRoot
         for x := 1 to len(pa_forAof) step 1
           cc := pa_forAof[x]
           aadd( pa_arg,  DBGETVAL(cc) )
         next

         filter := format(c_aof, pa_arg)
         (c_subTable)->( ads_setAof(filter), dbgoTop())
       endif
     endcase
   endif

   do while .not. (sCurrentTable)->( eof())
     Print_DefineData(adata,.f.)

     **  Print table line
     nRet := PrintLL_tableRow(sCurrentTable,hJob,adata)

     IF .not. empty(nRet); LLErrorMessage(nRet); Exit; EndIf

     ** move to next record
     (sCurrentTable)->(dbSkip())
   enddo

   nRet:= PrintLL_tableFooter(hJob)
   dbSelectArea(root_Table)
return nret


static function PrintLL_tableRow(sCurrentTable,hJob,adata)
   local nBuffer,nRet

   * print table line
   nRet := LlPrintFields(hJob)

   *  On page break print new header and repeat last data
   DO WHILE nRet = LL_WRN_REPEAT_DATA
     LlPrint(hJob)
     nRet := LlPrintFields(hJob)
   END DO
...
static function PrintLL_tableFooter(hJob)
  LOCAL nRet := LlPrintFieldsEnd(hJob)  // Print footer of last page
...
static procedure LLErrorMessage(nErrCode)
...
HIDDEN:
  ...
  VAR    cdirW
  METHOD SortOrder, Relations, Functions, ResetKey, SubTables, sort_subTable
...
METHOD LLIniFile:SortOrder(buffer)
  LOCAL  pa, isCompound, x, indexKey := '', n, nin
  *
  ...
  if( asc(buffer) <> 0 .and. (n := at(chr(0), buffer)) > 0 )
    ...
    *
    ...
    case(tagNo =  0 .and. .not. empty(indexKey))
      ...
      *
      ** onlyFor Mee  \\SERVER01SW
      if is_SERVER01SW
         nin  := at( 'Data', drgINI:dir_DATA )
         ::cWorkCdx_LL := subStr( drgINI:dir_DATA, 1, nin-1 )+ 'Users' +'\Tisky'
         ::cWorkCdx_LL := strTran( ::cWorkCdx_LL, ':6262', '' )
      else
  ...
  while(asc(buffer) <> 0 .and. (n := at(chr(0), buffer)) > 0)
    ...
    * za sekcí [Relations] mùže být další sekce
    if left(buffer,1) = '['
      ...
      *
      **  základní soubor MUSÍ bý otvøený, snad nebudeme vézat relaci na TMP
      if( .not. used(pa[4]), drgDBMS:open(pa[4]), nil )
...
METHOD LLIniFile:ResetKey(buffer)
  ...
  while(asc(buffer) <> 0 .and. (n := at(chr(0), buffer)) > 0)
    *
    * za sekcí [ResetKey] mùže být další sekce
    if left(buffer,1) = '['
      return self
    else
      cresetKey := substr(buffer,1,n -1)
  ...
  if n = 0  // za ResetKey už nic není, smyèka while vyskoèila ven
    cresetKey := buffer
    xresetKey := DBGETVAL(cresetKey)
  endif
return self
...
*
**
METHOD LLIniFile:SubTables(buffer)
   local  n    , lOk := .t.
   local  cname, clist
   local  nstep, pa
   local         pb, isOk, cvyraz, citem
   local  npos,  parent, subTable
   local         pc, x, cc, nlen, cfield_1, crel, cfield_2
   local  caof := ""
   *
   local  pa_it
   * pro test
   local  sub_Table, main_Table
   local  odbd, odesc_1, odesc_2, ctype_1, ctype_2

**   pa_subTables := {}

   while(asc(buffer) <> 0 .and. (n := at(chr(0), buffer)) > 0) .and. lOk
     if Left(buffer,1) = '['
       cname := lower(substr(buffer,2,n -3))
...
*
     ...
     for nstep := 1 to len(pa)-1 step 1
       ...
       case cvyraz = 'orderby'                   // zkusíme najít TAG, ale pokud neexistuje asi tøídit, na to bacha, položky musí existovat dbgDBMS:getFieldDesc(alias->cfield)
         pa_it[2] := citem
         if( .not. empty(citem), ::sort_subTable(pa_it, len(pa_subTables) +1) , nil )
//         if( .not. empty(citem), ::sort_subTable(pa_it, nstep) , nil )

       case cvyraz = 'restricton'                // soubor a za ním : (dvojTeèka) urèuje master pro omezení, lze použít klauzuli >, <, >=, <=, <>, = a spojení or nebo and
         caof     := ""
         npos     := at(':', citem )
         parent   := substr(citem, 1, npos-1)    // souobor musí existovat drgDBMS:getDBD(cfile)
         subTable := pa_it[1]
         odbd     := drgDBMS:getDBD(citem)
         pa_it[3] := parent
...
*           cfield_1-cfield_2 musí existovat, shodný typ, cfield_2 se pøesetuje na %%(N) nebo '%%'(C)
            ...
            cfield_2 := subStr(cc, npos+len(crel) )

            odesc_1 := drgDBMS:getFieldDesc(parent  ,cfield_1)
            odesc_2 := drgDBMS:getFieldDesc(subTable,cfield_2)

            if isObject(odesc_1) .and. isObject(odesc_2)
              ctype_1 := if( odesc_1:type = 'S', 'N', odesc_1:type )
              ctype_2 := if( odesc_2:type = 'S', 'N', odesc_2:type )
            ...
            caof  += cfield_2 +' '+crel+' ' +cc    //  cfield_2 pro format
            aadd( pa_it[5], parent +'->' +cfield_1 )
           endif
         next
     ...
     if( lOk, aadd( pa_subTables, pa_it ), nil )  // poslední krok na [EndSubTables]
     buffer := substr(buffer, n +1)
   enddo


   if len(pa_subTables) <> 0
     is_subTables := .t.  // public variable
     ::isVariable := .F.

     LlDbAddTable(hJob,"","")                               // Clear DLL-internal table buffer
     LlDbAddTable(hJob,"LLStaticTable","Freier Inhalt")     // add static table

     for x := 1 to len(pa_subTables) step 1
       sub_Table  := pa_subTables[x,1]
       main_Table := pa_subTables[x,3]

       drgDBMS:open(sub_Table)
       LlDbAddTable(hJob, sub_Table,"")

       (sub_Table)->(dbGoTop())
       DefineData(::isVariable,sub_Table,,::inDesign,::isdesc)

       if .not. empty(main_Table)
         drgDBMS:open(main_Table)
         (main_Table)->(dbGoTop())
         DefineData(::isVariable,main_Table,,::inDesign,::isdesc)

         LlDbAddTableRelation(hJob, sub_Table, main_Table, sub_Table +"_to_" +main_Table,"" )
       endif
     next
   endif
return self
...
method LLIniFile:sort_subTable(pa_it, ncnt)
  local  file := pa_it[1], buffer := pa_it[2]
  *
  local  pa, isCompound, x, indexKey := '', indexName := ''
  local  odesc, type, len, dec, tagNo := 0, tagName := ''
  local  npos, cc
  local  ctagFor_LL, cworkCdx_LL

  if select(file) <> 0                    // je otevøený, zapamatujem si tag
     pa_it[6] := (file)->(ordSetFocus())
  else                                    // nebo ho otevøeme
   drgDBMS:open(file)
  endif

//  pa         := ListAsArray(substr(buffer,1,n -1))        // chyba
  pa         := ListAsArray(buffer)
  isCompound := (Len(pa) > 1)
  *
  for x := 1 to len(pa) step 1
    cc := pa[x]
  ...
  *
  if len( (file)->(ordList())) <> 0
    pa_it[8] := indexName := (file) ->(Ads_GetIndexFilename())
  endif
  *
  if substr(upper(file), len(file), 1) <> 'W'
    indexDef    := drgDBMS:dbd:getByKey(file):indexDef
  ...
  **
  ...
  case(tagNo =  0 .and. .not. empty(indexKey))
    ...
    ** onlyFor Mee  \\SERVER01SW
    if is_SERVER01SW
      nin  := at( 'Data', drgINI:dir_DATA )
      cWorkCdx_LL := subStr( drgINI:dir_DATA, 1, nin-1 )+ 'Users' +'\TiskyS_' +strZero(ncnt,2)
      cWorkCdx_LL := strTran( cworkCdx_LL, ':6262', '' )
    else
      cWorkCdx_LL  := ::cdirW +'\TiskyS_' +strZero(ncnt,2)
    endif
...
METHOD LLIniFile:ReadSections( writeToFile )
  local  fields, pa, x, cvarName, npos
  local  lenBuff := 40960, buffer := space(lenBuff)
  ...
  if file( sName +sNameExt )
    ...
    * naèteme ze sekce [Description] RootTable *
    buffer  := space(lenBuff)

    GetPrivateProfileStringA('Description', 'RootTable', '', @buffer, lenBuff, sName +sNameExt)
    root_Table := substr(buffer,1,len(trim(buffer))-1)

    * naèteme ze sekce [UsedIdentifiers] Variables *
    buffer  := space(lenBuff)
...
METHOD LLIniFile:destroy()
  ...
  *
  local  sub_Table, old_tagName, new_adiFile, old_indexName
  ...
  * použil subTables
  if is_subTables
    for x := 1 to len(pa_subTables) step 1
      sub_Table     := pa_subTables[x,1]
      old_tagName   := pa_subTables[x,6]
      new_adiFile   := pa_subTables[x,7]
      old_indexName := pa_subTables[x,8]

      *
      ** tohle není dobøe
//      if( sub_table = 'kustree', nil, (sub_Table) ->( dbCloseArea()) )
      if sub_Table = 'kustree'
        if .not. empty(new_adiFile)
          (sub_Table) ->(OrdListClear())
          if( .not. empty(old_indexName), (sub_Table)->( OrdListAdd(old_indexName), ordSetFocus(1)), nil )
        endif
      else
        (sub_Table) ->( dbCloseArea())
      endif

      if .not. empty(new_adiFile)   // byl vytvoøen pomcný IDX, musíme ho zrušit
        do while FErase(new_adiFile +'.' +i_ext) = -1
        enddo
      endif

      if .not. empty(old_tagName)   // soubor byl otevøený, tak vrátíme pùvodní TAG
        drgDBMS:open(sub_Table)
        (sub_Table)->( ordSetFocus(old_tagName))
      endif
