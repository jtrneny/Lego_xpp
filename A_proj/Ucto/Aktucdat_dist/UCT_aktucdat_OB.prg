...
*
** AKTUALIZACE zùstatkù a OBratù na úètech z automatù***************************
  ...
  if ucetkum->(flock()) .and. ucetkumk->(flock()) .and. ucetkumu->(flock())
    do while .not. ucetkumw->(eof())
      ...
      if .not. ucetkum->(dbseek(ckeyS,,'UCETK_01'))
        ...
        /*
        ucetkum->cnazPol1    := SubStr(ckeyS, 13, 8)
        ucetkum->cnazPol2    := SubStr(ckeyS, 21, 8)
        ucetkum->cnazPol3    := SubStr(ckeyS, 29, 8)
        ucetkum->cnazPol4    := SubStr(ckeyS, 37, 8)
        ucetkum->cnazPol5    := SubStr(ckeyS, 45, 8)
        ucetkum->cnazPol6    := SubStr(ckeyS, 53, 8)
        */
