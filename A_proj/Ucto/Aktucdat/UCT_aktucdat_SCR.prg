...
*    drgDialog:dialog:drawingArea:options := XBP_IMAGE_SCALED
  ...
  *
  ** TAB 2
  inline method pushClose_or_OpenTask()
    ::setZavren_W()
  return .t.
  *
  ** TAB 3
  ...
  * bro col for ucetSys
  inline access assign method zavrel_kdoSy() var zavrel_kdoSy
    local  lzavren := ucetSys->lzavren
    local  d_Dat   := if( lzavren, ucetSys->duzvDat, ucetSys->dotvDat )
    local  c_Kdo   := if( lzavren, ucetSys->cuzvKdo, ucetSys->cotvKdo )
   return dtoc(d_dat) +'     ' +c_Kdo
  ...
  * bro col for ucetsys_W
  ...
  inline access assign method zavrel_kdoW() var zavrel_kdoW
    local  lzavren := ucetsys_W->lzavren
    local  d_Dat   := if( lzavren, ucetsys_W->duzvDat, ucetSys_W->dotvDat )
    local  c_Kdo   := if( lzavren, ucetsys_W->cuzvKdo, ucetsys_W->cotvKdo )
   return dtoc(d_dat) +'     ' +c_Kdo
  ...
  **
  inline method eventHandled(nEvent, mp1, mp2, oXbp)
    ...
    * zmìna období - budeme reagovat ale je na zmìnu ROKU
    case(nevent = drgEVENT_OBDOBICHANGED)
      ...
      if ::nrok <> uctOBDOBI:UCT:NROK
        ...
        if     ::tabNum = 1
          ::oDBrow[1]:oxbp:goTop():refreshAll()
        elseif ::tabNum = 2
          ::dc:sp_resetActiveArea( ::oDBrow[2] )
          ::dc:sp_resetActiveArea( ::oDBrow[3],,.f. )
        endif
        *
        aktucdatw->(dbzap())
  ...
  * filtr
  inline method setFilter()
    ...
    *
    ...
    ucetSys_B ->(ads_setaof(filter),dbgotop())

    ::rowPos := 1

    do while .not. ucetSys_B->(eof())
      if( ucetSys_B->naktUc_Ks = 2, ::rowPos++, nil )
      ucetSys_B->(dbskip())
    enddo

*    ::obro:deHilite()
*    ::obro:cursorMode := XBPBRW_CURSOR_NONE
    SetAppFocus(::obro)
...
METHOD UCT_aktucdat_SCR:Init(parent)
  ...
  drgDBMS:open('ucetsys',,,,,'ucetSys_B')  // pro napozicování BRo
...
method UCT_aktucdat_SCR:tabSelect(oTabPage,tabnum)
  ...
  do case
  case tabNum = 1
    ::dc:sp_resetActiveArea( ::oDBrow[1] )
...
method uct_aktucdat_scr:setZavren_W()
  ...
  *
  ...
  local  cInfo := 'Promiòte prosím,'                  +CRLF +CRLF + ;
                  'opravdu nelze otevøít/ zavøít %% ' +CRLF

  do case
  case( cfile = 'ucetsys'  )
    if ucetsys->naktuc_ks <> 2    // otevøít/zavøít úèetní období lze jen po aktualizaci
      ok   := .f.
      cerr := strTran( cinfo, '%%', 'úèetní období' )
      cerr += '  - není zpracována úèetní aktualizace -  '
    else
  ...
  case( cfile = 'ucetsys_w')
    if ucetSys->lzavren           // otevøít/zavøít období úlohy lze jen pokud je otevøeno úèetní období
      ok   := .f.
      cerr := strTran( cinfo, '%%', 'období úlohy' )
      cerr += padc( ' _ ' +allTrim( upper(::nazUlohy_W)) +' _', 45)  +CRLF +CRLF
      cerr += '  - je nutno otevøít úèetní období -  '
    else
      ok    := (cfile)->(sx_rLock())
      nlevl := 2
    endif
  ...
  if ok
    ...
    case( nlevl = 1 )
      ...
      uct_zavOtevr_kdo('ucetSys')

      if ucetsys->lzavren
        ucetsys_W->(dbEval( {|| ( ucetsys_W->lzavren := ucetsys->lzavren, ;
                                  uct_zavOtevr_kdo('ucetsys_W')           ) } ))
      endif
    ...
    if( .not. empty(cerr), fin_info_box( cerr, XBPMB_CRITICAL ), nil )
  endif
...
static function uct_zavOtevr_kdo(cfile)
  local  lzavren := (cfile)->lzavren

  if lzavren
    (cfile)->cuzvkdo    := usrName
    (cfile)->duzvdat    := date()
    (cfile)->cuzvcas    := time()
  else
    (cfile)->cotvkdo    := usrName
    (cfile)->dotvdat    := date()
    (cfile)->cotvcas    := time()
  endif
return .t.
