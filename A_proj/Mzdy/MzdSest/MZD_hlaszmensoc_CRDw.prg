...
*
*************** MZD_nem_prilzad ***********************************************
CLASS MZD_hlaszmensoc_CRDw FROM drgUsrClass
exported:
  var     task
...
method MZD_hlaszmensoc_CRDw:init(parent)
  local  task := 'mzd'
  ...
  drgDBMS:open('c_statpr')

  *
  drgDBMS:open('tmhlassow',.T.,.T.,drgINI:dir_USERfitm) ; ZAP
return self


method MZD_hlaszmensoc_CRDw:drgDialogStart(drgDialog)
...
method MZD_hlaszmensoc_CRDw:postValidate(drgVar)
...
method MZD_hlaszmensoc_CRDw:drgDialogEnd(drgDialog)
  ::msg   := ;
  ::dm    := ;
  ::dc    := ;
  ::df    := ;
...
method MZD_hlaszmensoc_CRDw:zpracuj_podklady()
  local  pa  := ::pa_obdZpr
...
*    tmhlassow->cUlicCiPop  := osoby->cUlicCiPop
    ...
    if c_staty->( dbSeek( Upper(osoby->cZkratStat),,'C_STATY1'))
      tmhlassow->cZkratStat  := c_staty->cZkratSta2
    endif
    ...
    if c_statpr->( dbSeek( Upper(osoby->cZkrStaPri),,'C_STATPR01'))
      if c_staty->( dbSeek( Upper(c_statpr->cZkratStat),,'C_STATY1'))
        tmhlassow->cZkrStatPr := c_staty->cZkratSta2
      endi
    endif
...
////   pøednastavení druhé strany pøílohy
  ...
  if msprc_mo->ntypduchod > 0
    if c_duchod->( dbseek( msprc_mo->ntypduchod,,'C_DUCHOD01'))
      ...
      tmhlassow->cTypDucReg := c_duchod->cTypDucReg
      tmhlassow->cDruhDucho := c_duchod->cTypDucReg
    endif
    tmhlassow->lDuchod := .t.
  endif
