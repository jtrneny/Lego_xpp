...
//-----+ FI_fakprihd_SCR +-------------------------------------------------------
CLASS MZD_mzdlisty_cp_SCR FROM drgUsrClass
EXPORTED:
//  VAR     KUHRADE_vzm    // k úhradì v základní mìnì

  METHOD  Init
  METHOD  ItemMarked, ItemMarkedOsb
//  METHOD  ItemSelected
  METHOD  InFocus
...
*********************************************************************
METHOD MZD_mzdlisty_cp_SCR:Init(parent)
...
METHOD MZD_mzdlisty_cp_SCR:InFocus(oB)
 ::drgDialog:DialogCtrl:oBrowse := oB:cargo
RETURN .T.

**
METHOD MZD_mzdlisty_cp_SCR:drgDialogStart(drgDialog)
  LOCAL nROK, nOBDOBI
  LOCAL cFiltr
...
METHOD MZD_mzdlisty_cp_SCR:ItemMarkedOsb()
  Local  n, nTabPage := 0
  ...
  cFiltr  := format( "crocp = '%%'", { msosb_mo->crocp } )
  mzdlisth->( ads_setAof( cfiltr ), dbgoTop())
  cFiltr  := format( "crocp = '%%' and nporpravzt = %%", { mzdlisth->crocp, mzdlisth->nporpravzt } )
  mzdlisti->( ads_setAof( cfiltr ), dbgoTop())

  aEVAL(dc:members[1]:aMembers,{|X| If( X:ClassName() = 'drgDBrowse', X:Refresh(.T.), NIL )} )

//  for q := 1 to len( dc:members[1]:aMembers)
//    if dc:members[1]:aMembers[q]:ClassName() = 'drgDBrowse'
//      dc:members[1]:aMembers[q]:refresh(.t.)
//    endif
//  next
...
METHOD MZD_mzdlisty_cp_SCR:ItemMarked()
  Local  n, nTabPage := 0
  Local  dc      := ::drgDialog:dialogCtrl
  Local  aValues := ::drgDialog:dataManager:vars:values, drgVar
  Local  cKy_BP
...
method MZD_mzdlisty_cp_SCR:aktMzdListy()
