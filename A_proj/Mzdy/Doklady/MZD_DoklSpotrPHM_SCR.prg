...
CLASS MZD_doklspotrphm_SCR FROM drgUsrClass
EXPORTED:
...
hidden:
  ...
  inline method setSysFilter( ini )
    ...
    cfiltr  := Format("nROKOBD = %% and nspotrphm <> 0", {rokObd}) + ::mo_prac_filtr
...
METHOD MZD_doklspotrphm_SCR:Init(parent)
...
METHOD MZD_doklspotrphm_SCR:InFocus(oB)
 ::drgDialog:DialogCtrl:oBrowse := oB:cargo
RETURN .T.


METHOD MZD_doklspotrphm_SCR:drgDialogStart(drgDialog)
  local  nposIn
...
METHOD MZD_doklspotrphm_scr:tabSelect(oTabPage,tabnum)
...
method MZD_doklspotrphm_scr:itemMarked(arowco,unil,oxbp)
...
METHOD MZD_doklspotrphm_SCR:ImportMzLOld(drgDialog)
...
**  MZD
method MZD_doklspotrphm_SCR:mzd_prackalendar_in()
  LOCAL oDialog
...
**  metoda pro volání pracovního kalendáøe
**  MZD
method MZD_doklspotrphm_SCR:mzd_importdokl_ml(drgDialog)
  local callFRM
  local key
...
**  MZD
method MZD_doklspotrphm_SCR:mzd_newObdNemocHD()
  local filtr
  local rok, mes
  local rokobd
...
method mzd_doklspotrphm_scr:postDelete()
  local  nsel, nodel := .f.
