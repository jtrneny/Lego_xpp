...
HIDDEN:
  ...
  var     zustDovBez, zustDovMin        // zùstatky bìžné dovolené vèetnì obobí
  var     zustDodBez, zustDodMin        // zùstatky dodatkové dovolené vèetnì obobí
  ...
  *
  ...
  inline method msprc_mo_seek()
    ...
    rokTm := Val( SubStr(cmain_Ky,1,4 ))
    obdTm := Val( SubStr(cmain_Ky,5,2 ))

    msprc_mo->( dbseek( cmain_Ky,, 'MSPRMO01'))

    if obdTm = 1
      cmain_Ky := StrZero( rokTm -1, 4) + '12' + SubStr(cmain_Ky,7 )
      msprc_mom->( dbseek( cmain_Ky,, 'MSPRMO01'))
    else
      cmain_Ky := SubStr(cmain_Ky,1,4 ) + StrZero( obdTm-1, 2) + SubStr(cmain_Ky,7 )
      msprc_mom->( dbseek( cmain_Ky,, 'MSPRMO01'))
    endif
  ...
  inline method sum_mzddavit()
    local  cdenik, npos, is_Ok
    local  pa  := ::pa_info
    *
    local  cmain_Ky := DBGetVal(::cmain_Ky)

    mzdDavIt_s->( ordSetFocus('MZDDAVIT01')      , ;
                  dbsetScope(SCOPE_BOTH,cmain_Ky), ;
                  dbgoTop()                        )

    ::dovolObd := 0

    do while .not. mzdDavIt_s->(eof())
      cdenik := mzdDavIt_s->cdenik
      ...
      if( npos:= ascan( pa, { |x| x[1] = cdenik })) <> 0
        ...
        * musíme ze souètu vylouèit opravovaný doklad a automat - jen denik MH a nautoGen = 1
        if( mzdDavIT_s->ndoklad = mzddavhdw->ndoklad )
          is_Ok := .f.
        else
          if( mzdDavIT_s->cdenik = 'MH' .and. mzdDavIT_s->nDnyDovol <> 0 )
            ::dovolObd += mzdDavIT_s->nDnyDovol
          endif
        endif
...
METHOD MZD_doklhrmz_CRD:Init(parent)
  ...
  * úroveò nákladové struktury ... Asystem++.ch
  ...
  drgDBMS:open('DRUHYMZD',,,,,'druhyMzd_s' )     // pro sumaci mzddavit
  ...
  * pro sumu za daný klíè
  ...
  drgDBMS:open('MZDDAVIT',,,,, 'mzddavIt_s')
  ::sum_mzddavhd()
  ::sum_mzddavit()
...
method MZD_doklhrmz_crd:ebro_saveEditRow( drgEBrowse )
  ...
  * nemocenská
  if (::hd_file)->cdenik = 'MN'
    ...
    * lVyloDobOD  -->  nDnyVylDOD
    ...
    (::it_file)->nHodFondKD := (::it_file)->nvykazN_KD * fPracDOBA()[3]
    (::it_file)->nHodFondPD := (::it_file)->nvykazN_PD * fPracDOBA()[3]
      ...
      do while .not. (::it_file)->(eof())
        if (::it_file)->nordItem >= nordItem  .and. (::it_file)->(recNo()) <> recNo
          ...
          ** pokud pøeèíslujeme, musíme pøeèíslovat i automaticky generované prémie
          if (::hd_file)->cdenik = 'MH' .and. isNull( (::it_file)->_nsidPrem, 0) <> 0
            if mzdDavitS->( dbseek( isNull( (::it_file)->_nsidPrem, 0),,'ID'))
  ...
  * srážky
  if (::hd_file)->cdenik = 'MS'
    ...
    (::it_file)->ntrvZavHd  := isNull( trvZavHd ->sID, 0)
