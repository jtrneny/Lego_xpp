...
*
*
** CLASS MZD_mzpobedo_IN ****************************************************
CLASS MZD_mzpobedo_IN FROM drgUsrClass, MZD_enableOrDisable_action
EXPORTED:
  ...
  * položky - bro
  inline access assign method is_genDoklad() var is_genDoklad
    return if( mzPobedo->nMZDDAVIT <> 0, MIS_CHECK_BMP, 0)
  ...
  * nautoGen 0 - mzdy 5 - výroba 6 - docházka
  ...
  inline method eventHandled(nEvent, mp1, mp2, oXbp)
    ...
    * zmìna období - budeme reagovat
    ...
    case nEvent = drgEVENT_DELETE
      if .not. mzPobedo->( eof()) .and. mzd_postSave()
        ::postDelete()
      endif
      return .t.
  ...
  inline method mzd_aktualizuj_Obedy()
...
hidden:
  ...
  inline method setSysFilter( ini )
    ...
    if ini
      ::drgDialog:set_prg_filter(cfiltr, 'mzpobedo')

    else
      if .not. empty(ft_APU_cond := ::drgDialog:get_APU_filter('mzpobedo', 'au') )
      ...
      mzpobedo->( ads_setaof(filtrs), dbGoTop())
...
METHOD MZD_mzpobedo_IN:Init(parent)
  ...
  drgDBMS:open('mzPobedo',,,,,'mzPobedoS')
  *
...
METHOD MZD_mzpobedo_IN:InFocus(oB)
 ::drgDialog:DialogCtrl:oBrowse := oB:cargo
RETURN .T.


METHOD MZD_mzpobedo_IN:drgDialogStart(drgDialog)
  local  nposIn
...
method MZD_mzpobedo_IN:itemMarked(arowco,unil,oxbp)
...
**  MZD
method MZD_mzpobedo_IN:mzd_prackalendar_in()
  LOCAL oDialog
...
**  MZD
method MZD_mzpobedo_IN:mzd_import_OBEDY(drgDialog)
  local callFRM
  local key
...
*/
    ...
    nsel := ConfirmBox( ,'Požadujete provést vytvoøení srážkových dokladù z podkladù pro obìdy', ;
...
method MZD_mzpobedo_IN:gen_doklady()
  ...
  * pro bìžné poøízení
  ...
  nrecCnt := mzpobedo->( ads_getKeyCount(1))
  nkeyCnt := nrecCnt

  do while .not. mzpobedo->( eof())
    if mzpobedo->nCenaObedy <> 0
      msPrc_moW->( dbseek( mzpobedo->nMSPRC_MO,, 'ID'))
  ...
  //    mzdDavhdw ->cObdobi    := ::cobdobi
      ...
      mzdDavhdw ->nMZPOBEDO  := isNull( mzpobedo->sID, 0)
      mzdDavhdw ->nautoGen   := 7
      *
      ...
      ** uložíme do dat originálu
      if mzpobedo->( sx_RLock())
        ...
        mzpobedo->nMZDDAVHD := isNull( mzdDavhd->sID, 0)
        mzpobedo->nMZDDAVIT := isNull( mzdDavit->sID, 0)

        mzpobedo->( dbUnlock(), dbCommit())
      endif
    endif
    mzpobedo->( dbskip())

    nkeyNo++
    if( mzpobedo->(eof()), nkeyno := nkeyCnt, nil )
    fin_bilancew_pb(::xbp_therm, nkeycnt, nkeyno)
  enddo

  if( .not. empty( mzpobedo->( ads_getAof())), mzpobedo->( ads_clearAof()), nil )
  ::Brow[1]:oxbp:refreshAll()
...
method mzd_mzpobedo_IN:postDelete()
  local  nsel, nodel := .f.
  local  ndel  := 0
  local  arecs := {}
  local  arSelect := ::Brow[1]:arSelect
  local  cFiltr   := mzPobedo->( ads_getAof())

  do case
  case      ::Brow[1]:is_selAllRec
    ok := mzPobedoS->(Flock())
    mzPobedoS->( ads_setAof(cFiltr), dbgoTop() )
  case len( arSelect) <> 0
    ok := mzPobedoS->( sx_Rlock( arSelect ))
    mzPobedoS->( ads_setAof(".f."), ads_customizeAof( arSelect,1), dbgoTop() )
  otherwise
    ok := mzPobedoS->( sx_Rlock( { mzPobedo->( recNo()) } ))
    mzPobedoS->( ads_setAof(".f."), ads_customizeAof( { mzPobedo->( recNo())} ,1), dbgoTop() )
  endcase
  ...
  mzPobedoS->( dbUnlock(), dbCommit(),ads_ClearAof(), dbgotop())
