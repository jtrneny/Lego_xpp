...
//-----+ FI_fakprihd_SCR +-------------------------------------------------------
CLASS MZD_doklnempr_SCR FROM drgUsrClass
EXPORTED:
...
*********************************************************************
METHOD MZD_doklnempr_SCR:Init(parent)
  ::drgUsrClass:init(parent)
/*
  drgDBMS:open('DRUHYMZD')
  drgDBMS:open('M_DAV')

  M_DAV->( DbSetRelation( 'DRUHYMZD',  { || M_DAV->nDruhMzdy },  'M_DAV->nDruhMzdy'))
 */
...
METHOD MZD_doklnempr_SCR:InFocus(oB)
 ::drgDialog:DialogCtrl:oBrowse := oB:cargo
RETURN .T.

**
METHOD MZD_doklnempr_SCR:drgDialogStart(drgDialog)
  LOCAL nROK, nOBDOBI
  LOCAL cFiltr
...
*****************************************************************
/*
METHOD MZD_doklnempr_SCR:ItemMarked()
...
METHOD MZD_doklnempr_SCR:ItemMarked()
  ...
  cKy_BP := StrZero( MSPRC_MO->nROK, 4)         ;
             +StrZero( MSPRC_MO->nOscisPrac, 5) +StrZero( MSPRC_MO->nPorPraVzt ,3)
  M_NEMOC->( AdsSetOrder(2), dbSetScope(SCOPE_BOTH, cKy_BP),dbGOTOP())

  cKy_NE := StrZero( M_NEMOC->nROK, 4)+ StrZero( M_NEMOC->nOBDOBI,2)           ;
          + StrZero( M_NEMOC->nOsCisPrac, 5)+ STRZERO( M_NEMOC->nPorPraVzt,3) + StrZero( M_NEMOC->nPoradi, 5)
  M_NEM->( AdsSetOrder(2), dbSetScope(SCOPE_BOTH, cKy_NE),dbGOTOP())
...
/*METHOD MZD_doklnempr_SCR:CardOfKmenMzd()
LOCAL oDialog
