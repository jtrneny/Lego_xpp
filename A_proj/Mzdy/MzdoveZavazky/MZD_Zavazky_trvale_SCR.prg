...
//-----+ FI_fakprihd_SCR +-------------------------------------------------------
CLASS MZD_trvalzav_SCR FROM drgUsrClass
EXPORTED:
...
*********************************************************************
METHOD MZD_trvalzav_SCR:Init(parent)
...
METHOD MZD_trvalzav_SCR:InFocus(oB)
 ::drgDialog:DialogCtrl:oBrowse := oB:cargo
RETURN .T.

**
METHOD MZD_trvalzav_SCR:drgDialogStart(drgDialog)
  LOCAL nROK, nOBDOBI
  LOCAL cFiltr
   /*
...
METHOD MZD_trvalzav_SCR:ItemMarked()
  Local  n, nTabPage := 0
  Local  dc      := ::drgDialog:dialogCtrl
  ...
  MZDY_OBD->( AdsSetOrder(1), dbSetScope(SCOPE_BOTH, cKy_BP),dbGOTOP())

  cKy_BP := StrZero( MZDY_OBD->nROK, 4)+ StrZero( MZDY_OBD->nOBDOBI, 2)           ;
          + StrZero( MZDY_OBD->nOscisPrac, 5) +StrZero( MZDY_OBD->nPorPraVzt, 3)
  MZDY->( AdsSetOrder(8), dbSetScope(SCOPE_BOTH, cKy_BP),dbGOTOP())
   */
/*
