...
function pro_explsthd_cpy(oDialog)
  ...
  ** tmp **
  ...
  if .not. lNEWrec
    mh_copyfld('explsthd', 'explsthdw', .T.)

    explstit->(dbgotop(), dbeval({|| mh_copyfld('explstit','explstitw',.t.)}))
  else
    explsthdw->(dbappend())
...
function pro_explsthd_wrt(odialog)
  ...
  explstitw->(ordsetfocus(0),dbgotop())
  ...
  if mainOk
    if(explsthdw->_delrec <> '9', mh_copyfld('explsthdw','explsthd',odialog:lnewRec), nil)
    explstitw->(dbgotop())

    do while .not. explstitw->(eof())

      if((nrecor := explstitw->_nrecor) = 0, nil, explstit->(dbgoto(nrecor)))
      if   explstitw->_delrec = '9'  ;  explstit->(dbdelete())
      else                           ;  mh_copyfld('explstitw','explstit',(nrecor=0))
      endif
...
function pro_explsthd_del(odialog)
  ...
  explsthdw->_delrec := '9'
  explstitw->(ordsetfocus(0),dbgotop(),dbeval({|| explstitw->_delrec := '9'}))
