...
#define  m_files   { 'firmy'   , 'c_staty', 'c_bankuc', ;
                     'dodlstit', 'objitem', 'vyrzakit', 'vyrzak' }

*
** CLASS for PRO_explsthd_IN ***************************************************

CLASS PRO_explsthd_IN FROM drgUsrClass, FIN_finance_IN, FIN_fakturovat_z_vld, FIN_PRO_fakdol
exported:
  ...
  * položky - bro
  ...
  inline method tabSelect(oTabPage,tabnum)
    ...
    case(otabPage:tabNumber = 2)   // 1 -> 2
      ::p_head:hide()
      if( (::it_file)->(eof()),  PostAppEvent(drgEVENT_APPEND,,,::brow), nil )
  ...
  inline method eventHandled(nevent,mp1,mp2,oxbp)
    ...
    case (nEvent = xbeBRW_ItemMarked)
      ...
      if( ::state <> 2, ::enable_or_disable_items( .f. ), nil )
      SetAppFocus(::brow)
      ::dm:refresh()
      return .f.
    otherwise
...
HIDDEN:
  ...
  *
  **
  inline method enable_or_disable_items( lstate )
    ::sklPol:odrg:isEdit    := lstate
    if( lstate, ::sklPol:odrg:oxbp:enable(), ::sklPol:odrg:oxbp:disable() )

    ::cisloDl:odrg:isEdit   := lstate
    if( lstate, ::cisloDl:odrg:oxbp:enable(), ::cisloDl:odrg:oxbp:disable())

    ::cislObInt:odrg:isEdit := lstate
    if( lstate, ::cislObInt:odrg:oxbp:enable(), ::cislObInt:odrg:oxbp:disable())

    ::cisZakazi:odrg:isEdit := lstate
    if( lstate, ::cisZakazi:odrg:oxbp:enable(), ::cisZakazi:odrg:oxbp:disable())
  return self
ENDCLASS
...
METHOD PRO_explsthd_IN:drgDialogStart(drgDialog)
  ...
  * projka FAKPRIHD-DODLSTHD-EXPLSTH
...
method PRO_explsthd_in:postAppend()
  local  dvykladky  := ::dm:get('explsthdw->dvykladky' )
  local  ccasvyklad := ::dm:get('explsthdw->ccasvyklad')
  ...
  ::dm:set('explstitw->dvykladky' , dvykladky  )
  ::dm:set('explstitw->ccasvyklad', ccasvyklad )
return .t.


method PRO_explsthd_in:overPostLastField(in_spcykl)
  local  ok       := .t.
...
method pro_explsthd_in:fir_firmy_sel(drgDialog)
  ...
  *
  ...
  ok := firmy->(::drgDialog:sel_dbseek( sel_ky, value,,'FIRMY1'))
//  ok := firmy->(dbseek(value,,'FIRMY1'))
  ...
  if copy
    ...
    case(file = ::hd_file)  ;  (::hd_file)->ncisFirDOP  := firmy->ncisFirmy
      ...
      ::df:setnextfocus('explsthdw->dnakladky',,.t.)
