...
#define  m_files   {'typdokl' ,'c_typoh'                        , ;
                    ...
                    'cenzboz' ,'cenprodc','procenho', 'nabvysit'  }
...
method pro_objhead_in:drgDialogStart(drgDialog)
  ...
  **
  ...
  if isCharacter( c_PRO_objhea := sysConfig('prodej:PRO_objhea'))
    pa := asize( listAsArray( strTran(c_PRO_objhea,' ', '')), 3)
...
METHOD PRO_objhead_IN:postValidate(drgVar)
  ...
  *
  local  nevent := mp1 := mp2 := oxbp := nil, isF4 := .F., ovar, recNo
...
*    case(name $ it_sel .and. mp1 = xbeK_RETURN )
*      ok    := ::objhead_z_sel()
...
*
  ...
  {      'ddatdoodb',  'objheadw->ddatdoodb',  'objheadw->ddatdoodb' }, ;
  {     'ddatodvvyr', 'objheadw->ddatodvvyr', 'objheadw->ddatodvvyr' }, ;
