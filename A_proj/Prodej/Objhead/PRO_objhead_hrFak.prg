...
EXPORTED:
    ...
    * F4
    ...
    if ( name = 'm->ddatdo_od' .or. name = 'm->ddatdo_do' )
      ...
      case( .not. empty(::ddatDO_od) .and.       empty(::ddatDO_do) )
        ...
        ::c_objhead_inRange := "objhead->ddatObj >= ctod('" +dtoc(::ddatDO_od) +"')"

      case(       empty(::ddatDO_od) .and. .not. empty(::ddatDO_do) )
        ::caof_objitem_new := format( "ddatDOodb <= '%%'", {::ddatDO_do} )
        ::c_objhead_inRange := "objhead->ddatObj <= ctod('" +dtoc(::ddatDO_do) +"')"
      ...
      otherwise
        if( ::ddatDO_od > ::ddatDO_do )
          ...
          ::c_objhead_inRange := "objhead->ddatObj >= ctod('" +dtoc(::ddatDO_od) +"') .and. " +"objhead->ddatObj <= ctod('" +dtoc(::ddatDO_do) +"')"
        endif
      endcase
      ...
      if ( name = 'm->ddatdo_do' )
        if(nevent = xbeP_Keyboard .and. mp1 = xbeK_RETURN)
          if ::caof_objitem <> ::caof_objitem_new
            ...
            // 19.2.2018 p.Gabriel chce vidìt všechno nevykryté
  ...
  inline method btn_START()
    ...
    *
    local  c_inRange    := ::c_objhead_inRange
    // 19.2.2018 p.Gabriel chce vidìt všechny nevyfakturované položky k firmì,
    // ale jinou barvou mimo rozsah zadání OD-DO
    local  c_inRange_it := strTran(c_inRange, 'objhead', 'objitem' )
    ...
    if len(anFIRMY) <> 0
      ...
      for nin := 1 to len(anFIRMY) step 1
        ...
        ( objheadWW->nmnozOBodb := 0, objheadWW->nmnozPLodb := 0, ;
          ...
          objheadWW->ninRange   := if( empty(c_inRange), 1, if( DBGetVal(c_inRange), 1, 2 ))
        ...
        do while .not. objitem->(eof())
          ...
          objitemww->ninRange   := if( empty(c_inRange_it), 1, if( DBGetVal(c_inRange_it), 1, 2 ))
...
HIDDEN:
  var  dm
  var  caof_objitem, caof_objitem_new, c_objhead_inRange
...
function PRO_objitemWW_colorBlock( a, b, c )
  ...
  *
  ...
  if useVisualStyle .and. IsThemeActive(.T.)
    if objheadWW->nmaxDoklad <> objitemWW->nDoklad .or. objitemww->ninRange = 2
      if objitemww->ninRange = 2
        return { , GraMakeRGBColor( {255, 255, 174 } ) }
      else
    ...
    if objheadWW->nmaxDoklad <> objitemWW->nDoklad .or. objitemww->ninRange = 2
      if( objitemww->ninRange = 2, aCOL_er[1] := GraMakeRGBColor( {23, 180, 255 } ), nil )
      return aCOL_er
    else
      return aCOL_ok
    endif
...
static function obj_browseContext(obj, ix, nMENU)
  return {|| obj:obj_fromContext( ix, nMENU) }
...
***************************  uvidíme ***
  ...
  inline method drgDialogStart(drgDialog)
     ...
     * 13.11.2017 - další požadavek p. Gabriela
     * zobrazit OBJHEADww v rozsahu zadání OD-DO, mimo rozsah, a já ještì pøidal v rozsahu i mimo rozsah
      for x := 1 TO Len(members) step 1
        if members[x]:ClassName() = 'drgPushButton'
          if( members[x]:event = 'nevykrObj_createContext', ::obtn_createContext := members[x], nil )
        endif
      next

      ::obtn_createContext:oxbp:setFont(drgPP:getFont(5))
      ::obtn_createContext:oxbp:SetGradientColors( { 0,5 } )     // {0,5} -zelená, {0,3} - èervená, {0,7} - žlutá

*      if( empty( objitem->( ads_getAof())), ::obtn_createContext:oxbp:hide(), nil )
      objheadWW->( ads_setAof( "ninRange = 1" ), dbgoTop())
  return self

  * 13.11.2017 - další požadavek p. Gabriela
  * zobrazit OBJHEADww v rozsahu zadání OD-DO, mimo rozsah, a já ještì pøidal v rozsahu i mimo rozsah
  inline method nevykrObj_createContext()
    local  opopup
    local  x, aPos, aSize
    local  pa_context := { { 'V rozsahu', 1, {0,5}, "ninRange = 1" }, { 'Mimo rozsah', 2, {0,3}, "ninRange = 2" }, { 'Kompletní seznam', 0, {0,7}, '' } }
    ...
    for x := 1 to len(pa_context) step 1
      opopup:addItem( {pa_context[x,1]                        , ;
                       obj_BrowseContext(self,x,pa_context[x]), ;
                                                              , ;
                            XBPMENUBAR_MIA_OWNERDRAW         }, ;
                      if( x = ::ninRange, 500, 0)               )
    next

    aPos    := ::obtn_createContext:oXbp:currentPos()
    aSize   := ::obtn_createContext:oXbp:currentSize()
    opopup:popup( ::obtn_createContext:oxbp:parent, { apos[1] -2, apos[2] } )
  return self


  inline method obj_fromContext(aorder,p_popUp)

    ::obtn_createContext:oxbp:setCaption( p_popUp[1] )
    ::obtn_createContext:oxbp:SetGradientColors( p_popUp[3] )

    if ::ninRange <> p_popUp[2]
      ::ninRange := p_popUp[2]

      if ::ninRange = 0
        objheadWW->( ads_clearAof(), dbgoTop())
      else
        objheadWW->( ads_setAof( p_popUp[4] ), dbgoTop())
      endif
...
HIDDEN:
  ...
  *
  ...
  var    ninRange, obtn_createContext
  ...
  inline method fakvyshdW_reFill()
    ...
    * jen pro jistotu, 8 - parametr je cvypSAZdan
    ...
    c_typUhr->( dbseek( upper(objheadWW->czkrTYPuhr),,'TYPUHR1' ))
    fakvyshdW->nkodZaokr := c_typUhr->nkodZaokr
  ...
  inline method postDelete(inFile)
    ...
    *
    ...
    local  paButton  := { '   ~Ano    ', ' Ano ~S vykrytím ', '    ~Ne   ' }
    local  csid_objitem := '', c_doklad_objhead := ''
      ...
      if nsel = 1 .or. nsel = 2
        if inBro = 1
          ...
          do while .not. objitemWW->( eof())
            ...
            objitemWds->npocOdb  := objitemWds->npocOdb   -1
            *
            ** pro rušení s vykrytím
            csid_objitem += strTran( str(objitemWW->_nsidOr), ' ', '' ) +','
            cdoklad      := strTran( str(objitemWW->ndoklad), ' ', '' )
            if .not. ( cdoklad $ c_doklad_objhead )
              c_doklad_objhead += cdoklad +','
            endif
        ...
        if( nsel = 2 .and. len(csid_objitem) <> 0 .and. len(c_doklad_objhead) <> 0 )
          csid_objitem     := left( csid_objitem    , len( csid_objitem)     -1)
          c_doklad_objhead := left( c_doklad_objhead, len( c_doklad_objhead) -1)

          ::vykr_objit_hd(csid_objitem, c_doklad_objhead)
        endif
  ...
  inline method vykr_objit_hd(csid_objitem, c_doklad_objhead)
    ...
    local stmt := "update objItem set nmnozPLodb = nmnozOBodb, " + ;
                                     ...
                                     "nstav_Svyd = 2 "           + ;
                          "where sID in ( %sid_objitem );"       + ;
                  "update objHead set nmnozPLodb = mnozPLodb, "  + ;
                                     "ddatRvykr  = curdate() "   + ;
                          "from "                                + ;
                          "( select ndoklad, sum(nmnozPLodb) mnozPLodb " + ;
                             "from objitem where objitem.ndoklad in ( %doklad_objhead ) group by ndoklad ) b " + ;
                          "where objhead.ndoklad in ( %doklad_objhead );"

    cStatement := strTran(       stmt, '%sid_objitem'   , csid_objitem     )
    cStatement := strTran( cStatement, '%doklad_objhead', c_doklad_objhead )
