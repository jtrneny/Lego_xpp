...
*
** CLASS for PRO_aktproceny_IN ************************************************
CLASS PRO_aktproceny_IN FROM drgUsrClass, FIN_finance_IN
exported:
  *
...
method PRO_aktproceny_IN:init(parent)
...
METHOD PRO_aktproceny_IN:drgDialogStart(drgDialog)
...
method PRO_aktproceny_IN:drgDialogEnd(drgDialog)
//  (::it_file)->(DbCloseArea())
//  (::hd_file)->(DbCloseArea())
return


method PRO_aktproceny_IN:postValidate(drgVar)
...
method PRO_aktproceny_IN:pro_firmy_sel(drgDialog)
...
method PRO_aktproceny_in:pro_aktproceny_gen()
  ...
  ** nachystáme si èervíka v samostatném vláknì
  ...
  oThread:start( "PRO_aktproceny_in_animate", xbp_therm, aBitMaps)
...
procedure PRO_aktproceny_in_animate(xbp_therm,aBitMaps)
...
method PRO_aktproceny_in:cmp_pc()
  ...
  *
  ...
  cky_1  := '00001' +strZero( cisFirmy,5) +upper( cisSklad) +upper( sklPol)
