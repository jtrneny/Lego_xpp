...
*  local  filtr, flt := "(ccissklad = '%%' .and. nmnozdzbo <> 0 .and. ncenapzbo <> 0 .and. laktivni)"
  ...
  *
  ...
  ::on_firmySel    := if( len(pa) >= 8, pa[8], '0')        //  0 - èíslo firmy není povinné , 1 - èíslo firmy je povinné
  ::on_basketSave  := if( len(pa) >= 9, pa[9], '0')        //  0 - ukládá na nfaktMnKoe, 1 - ukládá na czkrJednD, 2 - ukládá na ncejPrKDZ
...
* ::obdobi_Fin
  ...
  if ::lnewrec
    ...
    if (::hd_file)->cobdobi <> obdobi
      ConfirmBox( ,'Je mì líto, '                                                                        +CRLF + ;
                   'nelze spustil tuto nabídku, nejsou splnìny podmínky pro zpracování '                 +CRLF + ;
                   'aktuální datum ' +dtoc(dvystFak) +' <> od aktuálního období ' +(::hd_file)->cobdobi +' ...', ;
...
method pro_poklhd_in:saveParagon()
  ...
  * kontrola duplicity dokladu, poøizují ve stejné øadì proti sobì
  if ::lnewRec
...
*
  ...
  *
  ** sleva nastavená/ zanulovaná na hlavièce má pøednost
...
method PRO_poklhd_pay:drgDialogStart(drgDialog)
  ...
  *
  ...
  next
  *
  ** zablokujem DOA pokud není nastavena --> FAA => DOA
...
method PRO_poklhd_pay:fir_firmy_sp_sel(drgDialog)
  ...
  * u DOD musí existovat v seznamu dodacích adres vazba na firmyVA
