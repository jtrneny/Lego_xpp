...
EXPORTED:
  ...
  VAR     likeName        // klauzulka LIKE není povinná
  ...
  VAR     distrib         // indikace disribuèního souboru TAG pro vazbu
  ...
  METHOD  getTagByOrder   // return cTagName for nTagNo
...
LOCAL like, aDBD, aRF, ref, x
  WHILE ( keyWord := _parse(@st, @value) ) != NIL
    ...
    CASE keyWord = 'DISTRIB'
     ::distrib   := upper( _getStr(value) )
...
method drgFile:getTagByOrder( nTagNo )
  local aindex   := ::indexDef

  if nTagNo <= len( aindex )
    return aindex[ nTagNo ]:cName
  endif
return 0
...
METHOD drgFile:open(lExc, lIsWork, cDirName, lReadOnly, cAlias, lNoCheck)
  LOCAL  cDir, nArea, cFile, cconnect, osession, nsecBeg
  *
  DEFAULT lIsWork  TO .F.
  ...
  IF ValType(aDesc[1]) = 'O'
    FOR x :=  1 TO Len(adesc) STEP 1
      ...
      CASE pao:type = 'N'
        DO CASE
        CASE pao:dec <> 0
          AAdd( adesc_dbd, {pao:name, pao:type, pao:adt_len, pao:adt_dec})
...
**        AAdd( adesc_dbd, {pao[DBS_NAME], 'F', 8, 0})
  ...
  * check if file exist
  If lIsWork
    *
    ** pøi opravì dokladu potøebujene sID ze základního souboru
    aadd( adesc_dbd, { '_NSIDOR', 'N', 11, 0 } )
