...
HIDDEN:
  ...
  METHOD  progressCreate, progressRefresh
ENDCLASS
...
* Handle service thread events
    WHILE (nEvent := AppEvent(@mp1, @mp2, @oXbp, 1) ) != xbe_None .and. .not. ::terminated
...
* Is action stil in focus and it is the same action
        ...
        IF cargo != NIL .AND. VALTYPE(cargo) = 'O' .AND. cargo:IsDerivedFrom('DrgAction')
          ...
          IF cargo:oXbp != NIL              // win 2000 problem
            if cargo:oxbp:className() <> 'XbpImageButton'
              ::actionInFocus := cargo
...
method drgServiceThread:checkTermination()
  if ::terminated
    ::quit()
  endif
return self
...
METHOD drgServiceThread:deactivateAction()
  IF ::actionInFocus != NIL
    ...
    if isObject( ::actionInFocus )
      if ::actionInFocus:isDerivedFrom( 'drgAction' )
...
// JS  IF oXbp = ::lastTipped; RETURN; ENDIF

* Get cargo of object and RETURN if not set
  if isMemberVar(oxbp, 'toolTipText')
    if empty(oxbp:toolTipText)
      return
    else
...
METHOD drgServiceThread:progressStart(comment, maxValue, parentDialog)
  ...
  if( isObject(::dialog), nil, ::progressCreate() )
...
** SL1 nestartuje vlákno, používá jen progress
method drgServiceThread:progressCreate()
