...
METHOD drgObject:keyBoard(nKey, mp2, oXbp)
  local nEvent_l := mp1_l := mp2_l := oXbp_l := nil

  if ::parent:keyHandled( nkey, mp2, oxbp )
    if ::drgDialog:oForm:oLastDrg = self .and. isNull( ::in_postValidate)

      ::in_postvalidate := 1
               nEvent_l := LastAppEvent(@mp1_l, @mp2_l, @oXbp_l)
    ...
    *
    ** new požadavek z Kovaru u položek typu DATE pokud jsou prázdné * -> DATE()
    if nKey = 42  // *
      if IsObject(oXbp:cargo) .and. ( oXbp:cargo:ClassName()  = 'drgGet'     .or. ;
                                      oxbp:parent:className() = 'XbpCellGroup'    )
        if valType(oXbp:value) = 'D' .and. empty(oXbp:value)
          oXbp:setData( date() )
...
**  ono je to vlastnì okno s fokusem
