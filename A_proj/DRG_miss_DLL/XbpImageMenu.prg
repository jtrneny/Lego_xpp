...
//
//  Copyright:
//   Alaska Software, (c) 1997-2006. All rights reserved.
//
//  Contents:
...
METHOD XbpImageMenu:Create( oParent, aPP, lVisible )
 ...
 LOCAL aBox
   ...
   ::FontHeight := 13 //  oFnt:height

   IF Len(::BarText) > 0
      ::BarFont := XbpFont():new( oPS )
      ::BarFont:create( "12.Arial Black CE" ) // "12.Arial Bold" )
 ...
 LOCAL aItemBG := {aInfo[4][1] + ::BarWidth + ITEM_SPACING,aInfo[4][2],;
   ...
   IF BAnd(aInfo[2], XBP_DRAWACTION_SELCHANGE) != 0
      IF BAnd(aInfo[3], XBP_DRAWSTATE_SELECTED) != 0
         ...
         IF BAnd(aInfo[3], XBP_DRAWSTATE_DISABLED) != 0
           aAAttrs[GRA_AA_COLOR] := GraMakeRGBColor( {192,192,192} )
         ELSE
           aAAttrs[GRA_AA_COLOR] := GraMakeRGBColor( {200,200,255} )
         ENDIF
