...
METHOD VYR_exportML:init(parent)
  ...
  *
  drgDBMS:open('LISTIT'   )
  drgDBMS:open('Osoby'    )   // 'MsPrc_MD' )
  drgDBMS:open('MsPrc_MO' )   // 'MsPrc_MZ'
  drgDBMS:open('VYRZAK'   )
  drgDBMS:open('C_PRIPL'  )
  drgDBMS:open('cNazPOL1' )
  drgDBMS:open('UCETSYS'  )
  drgDBMS:open('UCETPOL'  )
  drgDBMS:open('MZDDAVHD' )   //'M_DAV'    )
  ...
  *
  ...
  ::cExpObd    := StrZERO(uctOBDOBI:VYR:NOBDOBI,2) + '/' + STR( uctOBDOBI:VYR:NROK, 4)
...
METHOD VYR_exportML:destroy()
  ...
  ::aExport   := ::cExportML := ::cExpObd  := ::cExpStr := ;
  ::Info_export := ::cUloha    := ;
  NIL
RETURN self
      ...
      *  INDEX ON M_DAVx->cNazPol1 + M_DAVx->cNazPol2 + M_DAVx->cNazPol3 TAG NazPol
        ...
        IF ::nExportML == DoMEZD
           ...
           ** Zruší pøípadný pøedchozí pøenos
           MZDDAVIT->( AdsSetOrder( 'MZDDAVIT04'),  mh_SetScope( cKey ))
           DO WHILE !MZDDAVIT->( EOF())
             IF MZDDAVIT->cKmenStrPr = ::cExpStr
               IF MZDDAVIT->( dbRLock())
        ...
        ** Generování M_DAVx z ListIT
        ...
        ListIT->( mh_SetSCOPE( Upper( cObdPREN)))
          ...
          ListIT->( dbGoTOP())
          DO WHILE !ListIT->( EOF())
            Osoby->( dbSEEK( ListIT->nOsCisPrac,, 'OSOBY03'))
            VyrZAK->( dbSEEK( Upper( ListIT->cCisZakaz),, 'VYRZAK1'))
            lExp := NO
            **IF UPPER( OSOBY->cKmenStrPr) = UPPER( ::cExpSTR) .AND. OSOBY->lExport
            ////////////// cKmenStrPr, lExport   chyb9 v souboru OSOBY /////////////////
        ...
        IF ::nExportML == DoMEZD   //... Export úkolových mezd == Do modulu MZDY A_SYSTEM
          ...
          ** Aktualizace M_DAV souborem M_DAVs
          ...
          DO WHILE !M_DAVs->( EOF())
             ...
             lSEEK := MZDDAVIT->( dbSEEK( cKey,, 'M_DAV5'))
             lOK := IIF( !lSEEK                        , AddREC( 'MZDDAVIT') ,;
                    IIF( M_DAV->cKmenStrPr == ::cExpSTR, ReplREC( 'MZDDAVIT'), AddREC( 'M_DAV') ))
             IF lOK
                lExp := YES
                mh_CopyFLD( 'M_DAVs', 'MZDDAVIT' )
                MZDDAVIT->dDatPoriz  := dPrenos
                MZDDAVIT->nOrdItem   := M_DAVs->nOrdItem * 10
                MZDDAVIT->( dbUnlock())
             ENDIF
             M_DAVs->( dbSKIP())
             nCount++
  ...
  DO WHILE !M_DAVx->( EOF())
    ...
    ELSE
       M_DAVx->( dbSKIP( -1))
        ...
        DruhyMzd->( dbSEEK( M_DAVs->nDruhMzdy,, 'DRMZDY1'))
...
METHOD VYR_exportML:AppendITMs()
  *
  M_DAVx->cKmenStrPR := OSOBY->cKmenStrPr   //MsPrc_MD->cKmenStrPr
  M_DAVx->nOsCisPrac := ListIT->nOsCisPrac
  IF ::nStredVML == 3
    M_DAVx->cNazPOL1 := ListIT->cNazPOL1
  ELSEIF ::nExportML == DoMEZD
    M_DAVx->cNazPOL1   := IF( EMPTY( VyrZAK->cNazPOL1), OSOBY->cKmenStrPR, VyrZAK->cNazPOL1 )
  ELSEIF ::nExportML == DoUCTA
    C_Stred->( dbSEEK( Upper( OSOBY->cKmenStrPr),, 'STRED1'))
...
//  M_DAVx->JM         := ListIT->cPrijPrac
  M_DAVx->cPracovnik := LEFT( OSOBY->cOsoba, 25) + StrZERO( M_DAVx->nOsCisPrac)
  ...
  IF ::nExportML == DoMEZD
    * IF MsPrc_MZ->( dbSEEK( ListIT->nOsCisPrac,, 'MSPRMZ01'))
    IF MsPrc_MO->( dbSEEK( ListIT->nOsCisPrac,, 'MSPRMZ01'))
      M_DAVx->nTypZamVzt := MsPrc_MO->nTypZamVzt
      M_DAVx->nTypPraVzt := MsPrc_MO->nTypPraVzt
      M_DAVx->cMzdKatPra := MsPrc_MO->cMzdKatPra
      M_DAVx->nZdrPojis  := MsPrc_MO->nZdrPojis
      M_DAVx->nClenSpol  := IF( MsPrc_MO->nTypZamVzt = 2 .or. MsPrc_MO->nTypZamVzt = 3, 1, 0 )
    ENDIF
  ENDIF
RETURN NIL
