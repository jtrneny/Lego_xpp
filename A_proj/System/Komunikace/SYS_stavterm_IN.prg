...
CLASS SYS_stavterm_IN FROM drgUsrClass
EXPORTED:
  METHOD  itemSelected
...
METHOD SYS_stavterm_IN:init(parent)
...
method SYS_stavterm_IN:drgDialogStart(drgDialog)
...
METHOD SYS_stavterm_IN:itemMarked()
  local  filtr
...
* ok
method SYS_stavterm_IN:postAppend()
...
method SYS_stavterm_IN:CheckItemSelected(drgCheck)

return


method SYS_stavterm_IN:itemSelected(new)
  local  mod
  *
  if ::selForm
...
METHOD SYS_stavterm_IN:preValidate(drgVar)
  local  lOk := .T., odesc
...
method SYS_stavterm_IN:postValidate(drgVar)
...
method SYS_stavterm_IN:ebro_beforeAppend(o_ebro)
...
method SYS_stavterm_IN:ebro_afterAppend(o_ebro)
  local  cfile   := lower( o_EBro:cfile)

  ::dm:set( 'stavterm->ctask'     , c_termin->ctask)
...
method SYS_stavterm_IN:deleteTSK
  local ok := .f.
...
method SYS_stavterm_IN:ebro_saveEditRow(o_eBro)
  local aUsers
  local n
...
//  if( o_eBro:isAppend, stavterm->( mh_append()), stavterm->(dbRlock()))
  ::dm:save()
  if o_eBro:isAppend
    stavterm->ctask      := ::dm:get('stavterm->ctask')
...
//  userseuc->nCisForms := Val(SubStr(userseuc->cIdTask,5))
  mh_WRTzmena( 'stavterm', o_eBro:isAppend)
  stavterm->(dbUnlock())
