...
//      drgDump( filtr )
      ...
      do while .not. usersTsk->( eof())
        ...
        ctskBegin  := AllTrim( if( secs(usersTsk->ctskBegin) = 0 , time()   , usersTsk->ctskBegin ))
        ctskBegTm  := AllTrim( ctskBegin)

        firstRun   := usersTsk->nfirstRun      //     odremovat po pøetoèení
        ...
        njd_tskBegin  := ::julianDate_Time(dtskBegin, ctskBegTm)
        ...
        cprgObject    := AllTrim(usersTsk->cprgObject)
//        drgDump(cprgObject)
...
//      drgDump( ::pa_Task_list )
    ...
    inline class method julianDate_Time(dDate,cTime)
      ...
      njd  := njdn + ( Round((nHod)/24,6) + Round( nMin/1440, 6)  + Round( nsec/86400,6))
...
//          usrIdSW  := licAsys->nusrIdSW
          ...
          dataADRfi       := AllTrim(licAsys->cdataDir)

          * connect to the ADS server datová èást
          cadd_File     := drgINI:dir_DATA +'\' + drgINI:add_FILE

          cConnect      := "DBE=ADSDBE;SERVER=" + cadd_File +";ADS_COMPRESS_ALWAYS;" +AllTrim(drgINI:ads_SERVER_TYPE)+";"+cUSRasys
          oSession_data := dacSession():New( cConnect)

          * connect to the ADS server uživatelská èást
          cConnect      := "DBE=ADSDBE;SERVER="  +AllTrim(drgINI:dir_WORK) +";ADS_LOCAL_SERVER" +";"+cUSRasys
  ...
  drgDBMS:open( 'users' )
  users->( dbseek( Upper( 'ADMIN     '),,'USERS01'))
  usrName := users->cuser

  * OBJEKT pro práci s UCT_ucetsys  *
...
//          drgDump(patask)
          ...
          * datum a èas spuštìní
          if ( (paTask.njd_tskBegin <= njd_Current) .and. if( paTask.ntypRun = 1, .t., ( paTask.njd_tskDenBeg <= njd_Current .and. paTask.njd_tskDenEnd >= njd_Current ) ) )
            ...
            is_tskRun   := .t.
            do case
            case paTask.ctypObject = 'FCE_KOM' .or. paTask.ctypObject = 'FCE_PRG'
  ...
  *           oThread:start( , paTask.cprgObject, ::o drgMenu )
            endcase

            * spustili jsme úlohu
            if is_tskRun
              do case
              case paTask.ntypRun = 1
                timeTm := mh_FROMJULIANDATE( paTask.njd_tskBegin)[3]
                nn := timeSec( timeTm)
              ...
              case paTask.ntypRun = 2
                njd_tm  := ::julianDate_Time(paTask.dtskBegin, paTask.ctskBegin)
                ninc    := if( paTask.njd_tskBegin < njd_tm .and. paTask.dtskBegin = date(), 0, 1)

                paTask.dtskDenBeg    := paTask.dtskDenBeg + ninc
                paTask.njd_tskDenBeg := ::julianDate_Time(paTask.dtskDenBeg, paTask.ctskDenBeg)

                paTask.dtskDenEnd    := paTask.dtskDenEnd + ninc
                paTask.njd_tskDenEnd := ::julianDate_Time(paTask.dtskDenEnd, paTask.ctskDenEnd)

                paTask.dtskBegin     := paTask.dtskBegin + ninc
                paTask.njd_tskBegin  := ::julianDate_Time(paTask.dtskBegin, paTask.ctskBegin)
...
function modi_logTask( pa, typ)
  ...
  local ok
   ...
   if ok
     mh_CopyFld( 'usersTsk','usrTsLog',.t.)
     usrTsLog->cDatTim := DtoC( Date()) + ' ' + Time()

     do case
     case typ = 1     // start úlohy
       cX := 'Start úlohy - bude spouštìna     : ' + DtoC(pa.dtskBegin) + ' ' +  pa.ctskBegin + CRLF
       cX := cX + "*********************** START ***********************" + CRLF
     case typ = 2     // start úlohy
       cX := 'Zmìna úlohy - bude spuštìna  : ' + DtoC(pa.dtskBegin) + ' ' +  pa.ctskBegin + CRLF
       cX := cX + "============= RE-START ============="  + CRLF
     case typ = 3
       usrTsLog->cDatTimBeg := DtoC( Date()) + ' ' + Time()
       cX := 'Úloha byla spuštìna                   : ' + DtoC( Date()) + ' ' + Time() + '  [ '+ DtoC(pa.dtskBegin) + ' ' + pa.ctskBegin + '] ' +CRLF
       cX := cX + Replicate("-"       , 54) + CRLF
     case typ = 4
       usrTsLog->cDatTimEnd  := DtoC( Date()) + ' ' + Time()
       usrTsLog->cDatTimNext := DtoC(pa.dtskBegin) + ' ' + pa.ctskBegin
       cX := 'Konec úlohy                                 : ' + DtoC( Date()) + ' ' + Time() + CRLF
       cX := cX + 'Pøíští spuštìní                              : ' + DtoC(pa.dtskBegin) + ' ' + pa.ctskBegin + CRLF
     endcase

     usrTsLog->nUsersTsk := usersTsk->sID
     usrTsLog->mLogTask  := cX
     usrTsLog->( dbunlock(), dbCommit())
   endif
