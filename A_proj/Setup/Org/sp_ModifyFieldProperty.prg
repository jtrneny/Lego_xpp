...
*  2
** kotrola ADD - DBD
function check_dbd_data( oMLE )
  local  x
  local  pao_drgFile := drgDBMS:dbd:values, fileName, o_drgFile
  *
    ...
    if o_drgFile:lIsCheck .and. fileName <> 'LICASYS'
      ...
      * vybudujeme pomocnou strukturu pro kontrolu/ založení indexù
      ...
      if s_tables->(dbseek( padr(fileName,20)))
        ...
        cStatement := create_table(fileName, aext_data_struct )
      endif
...
static function create_table(fileName, aext_data_struct)
  local x, pao
  *
  local cDEFAULT
