...
EXPORTED:
  ...
  inline access assign method pokus() var pokus
    retur CRLF +'  Vítá Vás Prùvodce instalací ' +CRLF + ;
                '  produktu Asystem++'

  inline access assign method popis() var popis
    return 'Toto je Prùvoce instalací produktu Asystem++, ver 1.11.11'      +CRLF + ;
           'na Váš poèítaè.'                                                +CRLF + ;
           ''                                                               +CRLF + ;
           'Doporuèuje se ukonèit veškeré spuštìné aplikace pøed tím, než'  +CRLF + ;
           'budete pokraèovat. Pøedejdete tím výskytu pøípadných'           +CRLF + ;
           'problémù bìhem instalaèního procesu.'                           +CRLF + ;
           ''                                                               +CRLF + ;
           'Klepnìte na Další pro pokraèování, nebo na Storno pro'          +CRLF + ;
           'ukonèení Prùvodce instalací.'
...
DRGTABPAGE INTO drgFC CAPTION '' FPOS 0,0 SIZE 80,15  OFFSET 1,82 TABHEIGHT 0
    DRGMLE M->POKUS  INTO drgFC FPOS 22, 0 SIZE 58, 4 SCROLL 'N'
    oDRG:groups := 'M'
*    oDrg:rOnly  := .t.

    DRGMLE M->popis INTO drgFC FPOS 22, 4 SIZE 58, 10.8 SCROLL 'N'  // PP 2
*    oDrg:rOnly  := .t.

DRGEND INTO drgFC


*    DRGSTATIC INTO drgFC FPOS .5,12 SIZE 85.1, 3 STYPE XBPSTATIC_TYPE_RECESSEDLINE   // RAISEDBOX
*      oDrg:ctype := 1


*    DRGPUSHBUTTON INTO drgFC CAPTION '         ~Instalace         ' ;
*                  EVENT 'pushButtonClick' SIZE 28,1.2 POS .4  ,1.9             ;
*                  ICON1 119 ICON2 0 ATYPE 3

    DRGPUSHBUTTON INTO drgFC CAPTION 'Další >' ;
                  EVENT 'pushButtonClick' SIZE 12,1.1 POS 52, 15.5           ;
  ...
  FOR x := 1 TO LEN(members) step 1
    IF members[x]:ClassName() = 'drgPushButton'
      if isCharacter( members[x]:event )
        if members[x]:event = 'pushButtonClick'

          drgPush := members[x]

*          drgPush:oXbp:setFontCompoundName("11.Arial Bold Italic")
*          drgPush:oXbp:setColorBG( graMakeRGBColor({170, 225, 170}) )
        endif
      endif

    elseif members[x]:ClassName() = 'drgMle'
      drgMle := members[x]
