...
**
** CLASS for c_odpoc **********************************************************
CLASS c_odpoc FROM drgUsrClass
EXPORTED:
  METHOD  init
...
HIDDEN:
  ...
  * filtr
  inline method setSysFilter( ini )
    ...
    if ini
      ::drgDialog:set_prg_filter(cfiltr, 'c_odpoc')

    else
      if .not. empty(ft_APU_cond := ::drgDialog:get_APU_filter('c_odpoc', 'au') )
...
METHOD c_odpoc:drgDialogInit(drgDialog)
  LOCAL  aPos, aSize
...
METHOD c_odpoc:postLastField(drgVar)
