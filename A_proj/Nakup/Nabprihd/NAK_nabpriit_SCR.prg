...
#define m_files  { 'ucetsys' ,'uzavisoz','dphdada','dph_2001','dph_2004'            , ;
                   ...
                   'banvyphd','banvypit','pokladhd','pokladit','nabprihd'           , ;
                   'ucetpol' ,'parvyzal','dodlstit','vyrzak'  ,'objitem' ,'cenzboz'   }


*
** CLASS for PRO_dodlstit_SCR **************************************************
CLASS NAK_nabpriit_SCR FROM drgUsrClass, FIN_finance_IN
exported:
  var     lnewRec, oinf
  ...
  inline access assign method cnazMeny() var cnazMeny
    c_meny->( dbSeek( upper(nabprihd->czkratMenZ),,'C_MENY1'))
    return c_meny->cnazMeny

  inline access assign method cnazevStat() var cnazevStat
    c_staty->(dbSeek( upper( nabprihd->czkratStat),,'C_STATY1'))
...
METHOD NAK_nabpriit_SCR:Init(parent)
...
METHOD NAK_nabpriit_SCR:drgDialogStart(drgDialog)
  ::brow := drgDialog:dialogCtrl:oBrowse
RETURN


METHOD NAK_nabpriit_SCR:tabSelect(oTabPage,tabnum)
...
method NAK_nabpriit_scr:itemMarked()
  local  cKy := strZero(dodlstit->ndoklad,10)

  nabprihd->( dbSeek( dodlstit->ndoklad          ,,'NABPRIH4'))
