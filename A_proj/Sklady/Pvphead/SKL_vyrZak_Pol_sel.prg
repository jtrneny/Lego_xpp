...
*
** pvpHead, pvpItem 21 - SKL_PRI115 ... 61 - SKL_VYD215
*
* class skl_vyrZak_sel vyrZak  skl_vyrZak_sel.frm  skl_pvphead_in:skl_vyrZak_sel
********************************************************************************
class skl_vyrZak_sel from drgUsrClass
EXPORTED:
 var  o_parent_udcp , o_parent_dm, o_parent_dBro
 var  hd_file, it_file, ntypPvp
...
*    drgDialog:dialog:drawingArea:options := XBP_IMAGE_SCALED
   ...
   // 1 - prij nvyrSt <> 1 and (nmnPROhlvy -nmnSKLpri) <> 0
   // 2 - vyd  nvyrSt  = 2 and (nmnVYDzmon -nmnSKLvyd) <> 0
   //   - pùvodní varianta ale nvyrSt 1,2,3 ... 1 je root ... výrobek
   inline method itemMarked()
     local  ntypPvp := (::hd_file)->ntypPvp
     local  filtr, flt := "ccisZakaz = '%%' and "
     local  cisZakaz := vyrZak->ccisZakaz

     flt += if( ::ntypPvp = 1, "nvyrSt <> 1 and (nmnPROhlvy -nmnSKLpri) <> 0" , ;
                               "nvyrSt <> 1 and (nmnVYDzmon -nmnSKLvyd) <> 0"   )
...
* pvpItem 21 - SKL_PRI115 ... 61 - SKL_VYD215
*
* class skl_vyrPo_sel vyrPol  skl_vyrPol_sel.frm  skl_pvphead_main:skl_vyrPol_sel
********************************************************************************
CLASS SKL_vyrPol_sel FROM drgUsrClass
EXPORTED:
...
//    return (objitem->nmnozObOdb -objitem->nmnozPlOdb)
  ...
  ** smallBasket
  ...
  inline method post_drgEvent_Refresh()
    ...
    if ( ::o_dBro:oxbp = ::dc:oaBrowse:oxbp )   // in vyrPol
      ::sta_activeBro:oxbp:setCaption( 337 )
      ::state := 2
      ::o_parent_udcp:takeValue(::it_file, 'vyrPol', 6, ::dm)
...
method skl_vyrPol_sel:init(parent)
  ::drgUsrClass:init(parent)
...
method skl_vyrPol_sel:drgDialogStart(drgDialog)
  ...
  * noAuto refresh on dc10
  ...
  pa_tagKey := drgScrPos:getPos_forSel('SKL_vyrPol_SEL', drgDialog, ::in_file)
...
method SKL_vyrPol_sel:createContext()
  local  opopup
  local  pa    := ::a_popUp
...
method SKL_vyrPol_SEL:fromContext(aorder,nmenu)
...
method skl_vyrPol_sel:mark_doklad(drgDialog)
...
method skl_vyrPol_sel:save_marked()
  ::sp_saved := .t.
