...
*  VAR     cNazPol1, cNazPol2, cNazPol3, cNazPol4, cNazPol5, cNazPol6
  ...
  METHOD  SetNS, SaveNS
  METHOD  SKL_CENZBOZ_SEL, SKL_OBJVYSIT_SEL, SKL_OBJITEM_SEL, SKL_C_Sklad, SKL_C_UctSkp
  METHOD  SKL_C_ODPMIS, SKL_C_SKUMIS, SKL_PRIJDIM, SKL_VYDEJKY
...
* Local nDph := SeekKodDPH( ::dm:get( ::IT+'->nKlicDPH') )    //0
...
* 1 - øádek end
    ...
    *
    ...
    Case ( name = ::IT + '->nCenaDokl1' )
      IF IsObject( ::dm:has(::IT + '->nCenNapDod' ))
        ::dm:set( ::IT+'->nCenNapDod', IF( CENZBOZ->NCENASZBO = 0, value, CenZboz->nCenaSZbo ))
      ENDIF
    */
**start
    * 23.10.2008
    CASE ( name = ::IT + '->NCENADOKL1')
      nMn := KoefPrVC_MJ( ::dm:get( ::IT + '->cMJDokl1'),;
                          CenZboz->cZkratJedn,;
                          'CenZboz' )
    ...
    CASE ( NAME = ::IT + '->cMJDokl1')
      nMn := PrepocetMJ( ::dm:get( ::IT + '->nMnozDokl1'),;
                         value ,;
                         CenZboz->cZkratJedn, 'CenZboz' )
      ::dm:set( ::IT + '->nMnozPrDod', nMn )
      nMn := KoefPrVC_MJ( ::dm:get( ::IT + '->cMJDokl1'),;
      ...
      ENDIF
      /*
      IF ( nPos := ASCAN( { 100,110,120,130,117 }, PVPHEADw->nKARTA)) > 0
        * * MJ je v tìchto kartách poslední položkou
...
**      ENDIF
    ...
    *
    CASE ( NAME = ::IT+'->nCenaPZBO')   // PC bez danì
       ...
       * lOK := SKL_ProdCENA( ::dm:get('PVPITEM->nCenaPZBO') )
      ENDIF
    ...
    CASE ( NAME = ::IT+'->nCenaPDZBO')   // PC s daní
      ...
      *  lOK := SKL_ProdCENA( ::dm:get('PVPITEM->nCenaPDZBO'))
...
*          anMnR := ObjVyst_R( nKEY, PVPItem->nMnozPrDod )
        ...
        */
        ...
        /****
        IF( nKEY = xbeK_ENTER, SKL_ObjVyst_Modi( xbeK_DEL ), NIL )
        nMnHLP := MAX( ObjVysIT->nMnozObDOD - ObjVysIT->nMnozPlDOD, 0)
        PVPItem->nMnozVyObV  := MIN( PVPItem->nMnozPrDOD, nMnHLP )
        SKL_ObjVyst_Modi( xbeK_INS )
        SKL_Firmy_Modi( nKEY )
        SKL_MnozKzbo_Modi( nKEY)
        *****/
      ENDIF
...
**-     nSumaItem := SKL_NutneVN( ::drgDialog, nKEY, PVPItem->( RecNo()),, .t. ) // .f. )
    /*
    CASE ( nPos := ASCAN( { 102,142,103,104 }, ::nKARTA)) > 0
      * Produkce  a Výrobní produkce
      *IF SKL_VyrZAK_TST( PVPItem->cSklPOL, PVPItem->cNazPOL3)
        PVPItem->cCisZakaz  := PVPItem->cNazPOL3
        PVPItem->nCenaCelk  := IF( ::nKarta = 102, PVPItem->nMnozPrDod * PVPItem->nCenNapDod,;
                                                   MH_RoundNumb( PVPItem->nMnozPrDod * PVPItem->nCenNapDod, 12) )
        PVPItem->nCenapDZBO := PVPItem->nCenaCelk * ( 1 + ( SeekKodDph( PVPItem->nKlicDph) / 100))

        SKL_CenZboz_MODI( nKEY )
        SKL_VyrZAK_MODI( nKEY)
      *ENDIF
    */
...
* Nad cSklPol musí být výbìr z položek výdejového dokladu
  ...
  ** cenZboz / pvpTerm
  ...
  if .not. ( upper( flt_cenZboz) $ upper( cenZboz ->( ads_getAof())))
    CenZboz->( mh_SetFilter( flt_cenZboz ))
  endif
...
METHOD SKL_POHYBYIT:SKL_OBJVYSIT_SEL( Dialog)
  ...
  IF IsObject( Dialog) .or.  !Ok
    Filter := FORMAT( "StrZero(nCisFirmy,5) = '%%' .and. upper(cCisSklad) = '%%' .and. OBJVYSIT->nmnozobdod <> 0 .and. " + ;
              "(OBJVYSIT->nmnozpldod = 0 .or. (OBJVYSIT->nmnozpldod <> 0 .and. OBJVYSIT->nmnozpldod < OBJVYSIT->nmnozobdod))", {StrZero( (::HD)->nCisFirmy,5),(::HD)->cCisSklad })
    ObjVysIT->( mh_SetFilter( Filter))

    ObjVysIt_modiAOF()

    DRGDIALOG FORM 'SKL_OBJVYSIT_SEL' PARENT ::drgDialog  MODAL DESTROY ;
                                      EXITSTATE nExit
    ObjVysIT->( mh_ClrFilter())
  ENDIF
...
*      FOrdREC()
  ...
  *
*  ObjVysIT->( mh_ClrFilter())
*  If( ok, ObjVysIT->( dbGoTo( nRecObj)), nil )

RETURN OK

* Z filtru se vyøadí záznamy, které nesplòují podmínku  z ObjVysIT
*===============================================================================
FUNCTION ObjVysIT_modiAOF()
*
  Local nRec := PVPITEMww->( RecNo()), cKey, aRec := {}

  PVPITEMww->( dbGoTOP())
  DO WHILE ! PVPITEMww->( eof())
    cKey := Upper( PVPITEMww->cCisObj) + StrZero( PVPITEMww->nIntCount, 5)
    IF OBJVYSIT->( dbSeek( cKey,, 'OBJVYSI5' ))
*      IF ObjItem->nMnozObOdb - ObjItem->nMnozPlOdb - PVPITEMww->nMnozPrDod <= 0
      IF !( OBJVYSIT->nmnozpldod + PVPITEMww->nMnozPrDod = 0 .or. (OBJVYSIT->nmnozpldod + PVPITEMww->nMnozPrDod <> 0 .and. OBJVYSIT->nmnozpldod + PVPITEMww->nMnozPrDod < OBJVYSIT->nmnozobdod ))
        AADD( aRec, ObjVysIT->( RecNO()))
      ENDIF
    ENDIF
    PVPITEMww->( dbSkip())
  ENDDO
  ...
  IF Len( aRec) > 0
    OBJVYSIT->(Ads_customizeAOF(aRec, 2))
  ENDIF
*/
RETURN nil


* Výbìr skladové položky z OBJITEM ( objednávek pøijatých) do výdejového dokladu
********************************************************************************
/*
method skl_pohybyit:skl_objitem_sel(drgDialog)
  local  oDialog, nExit
  local  recCnt := 0, showDlg := .f., ok := .f., isOk := .f.
  local  cf := "ncisFirmy = %% .and. (nmnozObODB -nmnozPlOdb) > 0", cfilter
  local  removeRecs := {}
  //
  local  ncisFirmy  := if( ::nKarta = 274 .or. ::nKarta = 293, 1, (::hd)->ncisFirmy)
  local  ccislOBint := ::dm:get( ::it +'->ccislOBint')
  local  ncislPOLob := ::dm:get( ::it +'->ncislPOLob')

  cfilter := format( cf, { ncisFirmy } )
  objitem->( ads_setAof( cfilter ))

  fordRec( { 'pvpitemww' } )
  pvpitemww->( dbeval( { || ;
    ( cky := upper(pvpitemww->ccislOBint) +strZero(pvpitemww->ncislPOLob,5)  , ;
      if( objitem->( dbseek(cky,,'OBJITEM2')) .and.                            ;
...
METHOD SKL_POHYBYIT:SKL_OBJITEM_SEL(Dialog)
  ...
  *
  ...
  Local ok, okPos, nRecObj, oMoment, nMnozPlOdb := 0
  *
  ** Hydrap specifikum crozATRpro <> ROK ostatním by to nemìlo vadit
  local cf := "ncisFirmy = %% .and. (nmnozObODB -nmnozPlOdb) > 0 .and. upper(crozATRpro) <> 'ROK'"

  ObjItem->( mh_ClrScope(), mh_ClrFilter(), dbGoTOP())
  ObjItem->( AdsSetOrder(3), mh_ClrScope())
  okPos   := ( !Empty(value) .and. ObjITEM->(dbseek( cKEY,,'OBJITE10')))
  IF( okPos, NIL, ObjITEM->( dbGoTOP()) )
  nRecOBJ := ObjITEM->( RecNO())  //   IF( okPos, ObjITEM->( RecNO()), 1)
...
*    oMoment := SYS_MOMENT('Probíhá výbìr objednávek pøijatých ...')
    ObjItem->( mh_SetFilter( cFilter ))  // , nRecObj))
*    oMoment:destroy()
    ObjItem_modiAOF()

    * pokud je filtr prázdný, v SKL_OBJITEM_SEL se nepozicujeme na nrecObj
*    nrecObj := if( objitem->( eof()), 0, if( objitem->(ads_isRecordInAof(nrecObj)), nrecObj, 0) )
    ...
    DRGDIALOG FORM 'SKL_OBJITEM_SEL' CARGO 0 PARENT ::drgDialog  MODAL DESTROY ;
                                     EXITSTATE nExit
    ObjItem->( mh_ClrFilter())
  ENDIF
...
*  IF nExit = drgEVENT_QUIT
*    ::dm:set(::IT + '->CCISLOBINT',  '' )
*  ENDIF
RETURN ( nExit != drgEVENT_QUIT .or. ok)

* Z filtru se vyøadí záznamy, které nesplòují podmínku  z ObjITEM
*===============================================================================
FUNCTION ObjItem_modiAOF()
  Local nRec := PVPITEMww->( RecNo()), cKey, aRec := {}

  PVPITEMww->( dbGoTOP())
  DO WHILE ! PVPITEMww->( eof())
    cKey := Upper( PVPITEMww->cCislObInt) + StrZero( PVPITEMww->nCislPolOb, 5)
    IF OBJITEM->( dbSeek( cKey,, 'OBJITEM2' ))
      IF ObjItem->nMnozObOdb - ObjItem->nMnozPlOdb - PVPITEMww->nMnozPrDod <= 0
        AADD( aRec, ObjItem->( RecNO()))
      ENDIF
    ENDIF
    PVPITEMww->( dbSkip())
  ENDDO
  PVPITEMww->( dbGoTO( nRec))
  IF Len( aRec) > 0
    OBJITEM->(Ads_customizeAOF(aRec, 2), dbgoTop())
  ENDIF
  ...
  For x := 1 TO Len( ::membORG)
    ...
    IF IsMemberVar(oVAR,'Groups')
      ...
      ElseIf IsCharacter( oVAR:Groups)
          ...
          If EMPTY(  oVAR:Groups) .OR. ::uHd:cCRD $ oVAR:Groups .OR. 'clrYELLOW' $ oVar:Groups
  ...
  For x := 1 To LEN( ::varsORG:values)
    IF ! IsNIL( ::varsORG:values[x, 2] )
      ...
      IF oVAR:ClassName() $ 'drgGet,drgText,drgComboBox'
        If IsNIL( oVar:Groups) .OR. EMPTY(oVar:Groups) .OR. (::uHd:cCRD $ oVAR:Groups) .OR. 'clrYELLOW' $ oVar:Groups
...
/********************************************************************************
FUNCTION CompresFile( cDir, cSubDir, cReplace)
RETURN .T.
*/