...
#define m_files  { 'typdokl'   ,'c_typoh'                                    , ;
                   ...
                   'kurzit'    ,'fakprihd' ,'fakvyshd','ucetpol', 'mzdzavhd' , ;
                   'firmy'                                                   , ;
      ...
      if .not. (::typ_dokl $ 'pok')
        if .not. empty( (::hd_file)->cfile_imp)
          ...
          if ::lNEWrec .and. ::nLockH_im <> 0
            banvyph_im->(dbgoTo(::nLockH_im), dbRlock())
 ...
 * likvidace bankovní výpisy/vzájemné zápoèty/ úhrady pokladnou nemají RVDPH *
  ...
  *
  if( ::typ_dokl = 'ban', ::import_bavy_mh(), nil )
  ::showGroup()
  ::sumColumn()
  ::dm:refresh()
  ...
  * ukládáme na posledním PRVKU *
  ...
  if isChanged .and. if(::state = 2, addrec(::it_file), .T.)
    if ::state = 2  ;  cenzak_or := if( pos <> 0, (::it_file)->(fieldget(pos)), 0)
      ...
      *
      ** pøi opravì imortované položky která se nespárovala
      if ( .not. empty( file_iv) .and.                 ;
                 empty( (::it_file)->cdenik_Par) .and. ;
           .not. empty( (::hd_file)->cfile_imp )       )

        ::copyfldto_w(file_iv,::it_file)
        *
        ** si pøepíšeme cdenik a ndoklad pak zmizí položka z dokladu
    ...
    *
    if .not. empty(file_iv)
      (file_iv)->(dbseek((::it_file)->ncisFak,, AdsCtag(1)))
    endif

    ::postLastField_mh( file_iv )
    if( ::is_ban, (::it_file)->nerr_imp := 0, nil )
  ...
  if nexit != drgEVENT_QUIT
    ...
    do while .not. banvypi_im->(eof())
      if ( banvyph_im->nrok_vyp  = banvypi_im->nrok_vyp  .and. ;
        ...
        ** rozšíøíme podmnínku jen na nehrazene, nebo èásteènì uhrazené
        ** pokud jich najdeme víc zkusíme najít shodu pro úhradu z banky
        nin_file   := if( banvypi_im->ntypObratu = 2, 1, 2)
        cfile_in   := ain_file[ nin_file, 1]
        ctag_in    := ain_file[ nin_file, 7]
        bfor_in    := COMPILE( strTran( cfor_in, '...', cfile_in ))
        pa_recs_in := {}
        nsign      := +1

        (cfile_in)->( AdsSetOrder(ctag_in)         , ;
                      DbSetScope(SCOPE_BOTH,varSym), ;
                      DbgoTop()                    , ;
                      DbEval( { || aadd( pa_recs_in, (cfile_in)->(recNo()) ) }, ;
                                   bfor_in                                      ), ;
                      DbClearScope()                                               )

        *
        ** pokusíme se najít položky v mzdZavhd
        if len( pa_recs_in) = 0
           nin_file   := 3
           cfile_in   := ain_file[ nin_file, 1]
           ctag_in    := ain_file[ nin_file, 7]
        ...
        *
        ** blázinec s dobropisy, pokud pohle ntypObratu nic nenašel
        ** zkusíme to na druhé stranì
        if len( pa_recs_in) = 0
          nin_file   := if( nin_file = 1, 2, 1 )
          cfile_in   := ain_file[ nin_file, 1]
          ctag_in    := ain_file[ nin_file, 7]
          bfor_in    := COMPILE( strTran( cfor_in, '...', cfile_in ))
          pa_recs_in := {}
        ...
        *
        ** pokud o nenajdeme ani na druhé stranì musíme vrátit pùvodní nastavení
        ** dle ntypObratu
        if len( pa_recs_in) = 0
           nin_file   := if( banvypi_im->ntypObratu = 2, 1, 2)
           cfile_in   := ain_file[ nin_file, 1]
        ...
        do case
        case len(pa_recs_in) = 1
          (cfile_in)->( dbgoTo( pa_recs_in[1] ))
          *
          ** 21.7.2016 platby na státní instituce mají ve varSymbolu IÈO firmy
          ** párovat se mùže jen pokud se jedná o stejnou èástku
          if ::istuz
            do case
            case nico_cfg = val(varSym)
              file_iv := if( banvypi_im->ncenZakCel =  (cfile_in)->ncenZakCel, cfile_in, '' )
            otherwise
              file_iv := if( banvypi_im->ncenZakCel <= (cfile_in)->ncenZakCel, cfile_in, '' )
            endcase
            ...
            do case
            case nico_cfg = val(varSym)
              file_iv := if( banvypi_im->ncenZahCel =  (cfile_in)->ncenZahCel, cfile_in, '' )
            otherwise
              file_iv := if( banvypi_im->ncenZahCel <= (cfile_in)->ncenZahCel, cfile_in, '' )
            endcase
          endIf
...
/*
          do case
          case ::istuz
            if banvypi_im->ncenZakCel <= (cfile_in)->ncenZakCel
              file_iv := cfile_in
            endif
          otherwise
            if banvypi_im->ncenZahCel <= (cfile_in)->ncenZahCel
              file_iv := cfile_in
            endif
          endCase
*/
        ...
        case len(pa_recs_in) > 1
          begin sequence
            for nstep := 1 to len(pa_recs_in) step 1
              (cfile_in)->( dbgoTo( pa_recs_in[nstep] ))
        ...
        * dobropis ? má obrácené znaménko na nuhrcelFAK / FAZ
        (::it_file)->nuhrcelFak := (::it_file)->nuhrcelFak * nsign
        (::it_file)->nuhrcelFaz := (::it_file)->nuhrcelFaz * nsign
...
method FIN_ban_vzz_pok_IN:import_bavy_mh( neco )
  ...
  *
  ...
  local  xkey      := strZero((::hd_file)->nrok,4) +upper( (::hd_file)->cbank_uce) +'0'
  *
  ** pro ASys_Komunik(typ,::drgdialog)
  ...
  if ::is_ban
   ...
   if datkomhd->(dbseek( cIdDatKom,, 'DATKOMH01'))
     ...
     GetPrivateProfileStringA('Import_usr', 'pathImport', '', @buffer, lenBuff, sName)
     ::cpath_kom := substr(buffer,1,len(trim(buffer))-1)

     buffer  := space(lenBuff)
     GetPrivateProfileStringA('Import_usr', 'datUhrZhl', '', @buffer, lenBuff, sName)
     ::datUhrZhl := substr(buffer,1,len(trim(buffer))-1)
   ...
   *  1. - zkusíme najít nastavení pro import
   if datkomhd->(dbseek( cIdDatKom,, 'DATKOMH01'))
     ...
     ** ::odata se použijí pøi volání ASYS_Komunik
     pa_mDatkom_us := listAsArray( memoTran( datkomhd->mDefin_kom,,''),';')
    ...
    oClass  := RecordSet():createClass( "selectkom_crd_" + datkomhd->cIDdatKom, pa_items )
...
*/
   ...
   if ( len( afiles ) <> 0 .or. banvyph_im->( dbseek( xkey,,'BANIMPH_1')))
     (::drgVar_file_imp:odrg:isEdit := .T., ::drgVar_file_imp:odrg:oxbp:enable())
      ::drgVar_file_imp:odrg:pushGet:oxbp:enable()
   else
     (::drgVar_file_imp:odrg:isEdit := .F., ::drgVar_file_imp:odrg:oxbp:disable())
      ::drgVar_file_imp:odrg:pushGet:oxbp:disable()
   endif
...
*** BACHA
    ...
    ok := FIN_banvyp_wrt_inTrans(self)
  endIf
