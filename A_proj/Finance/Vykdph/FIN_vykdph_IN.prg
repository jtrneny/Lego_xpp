...
*
** CLASS for FIN_vykdph_IN *****************************************************
CLASS FIN_vykdph_IN FROM drgUsrClass
exported:
  var     nFINTYP
  *
  var     mainFile
  ...
  inline access assign method oddil_kohl var oddil_kohl
   ...
   inline method comboBoxInit(drgCombo)
     ...
     if ( cname = 'coddilkohl' )
       acombo_val := FIN_c_vykdph_coddilKohl(.t.)
...
method FIN_vykdph_IN:init(parent)
...
method FIN_vykdph_IN:drgDialogStart(drgDialog)
  local  x, nIn, cfield
...
function FIN_vykdph_in_colorBlock( a, b, c )
...
method FIN_vykdph_in:drgDialogEnd()
  vykdph_iw->(dbclearfilter())
  ::sum()
return self


method FIN_vykdph_IN:itemMarked()
*-  ::showCell()
return self
...
*          3 Snížená daò         1
*          4 Základní daò        2
method FIN_vykdph_IN:tabSelect(drgTabPage, tabNum)
...
method fin_vykdph_in:prevalidate(drgvar)
...
method fin_vykdph_in:postValidate(drgVar)
...
METHOD FIN_vykdph_IN:postLastField(drgVar)
...
method FIN_vykdph_IN:FIN_cmdph(drgDialog)
