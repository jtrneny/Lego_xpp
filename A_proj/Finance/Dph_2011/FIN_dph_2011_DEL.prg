...
*
** CLASS for FIN_dph_2011_DEL ***************************************************
CLASS FIN_dph_2011_DEL FROM drgUsrClass
EXPORTED:
  var     zprac, dotaz
  method  init, destroy, drgDialogStart, dph_Delete

  inline access assign method dotaz() var dotaz
    return 'Zrušit daòovou uzávìrku za období _ ' +str(dph_2011->nobdobi,2) +'/' +str(dph_2011->nrok,4) +' _'
...
METHOD FIN_dph_2011_DEL:init(parent)
  ::drgUsrClass:init(parent)

  ::zprac  := 0
  ::dotaz  := 'Zrušit daòovou uzávìrku za období _' +DPH_2011 ->cOBDOBIDAN +'_'
...
METHOD FIN_dph_2011_DEL:destroy()
  ::drgUsrClass:destroy()
...
METHOD FIN_dph_2011_DEL:drgDialogStart(drgDialog)
  local x, pA, members  := drgDialog:oForm:aMembers, showDialog
  *
  local ky := upper(dph_2011->culoha) +strZero(dph_2011->nrok,4) +strZero(dph_2011->nobdobi,2)
  ...
  showDialog := (dph_2011->(rlock()) .and. ;
                 ucetsys ->(rlock()) .and. ;
                 prijatpl->(flock()) .and. uskutpl->(flock()))
  showDialog := (showDialog .and. .not. dph_2011->(eof()))
...
**
method FIN_dph_2011_DEL:dph_Delete()
  local  x, nor, cin, cky := DPH_2011 ->cOBDOBIDAN, oXbp, ostate, prc
  *
  local  nreccnt, nkeycnt, nkeyno
  local  cc    := str(dph_2011->nobdobi,2) +'/' +str(dph_2011->nrok,4)
  ...
  if dph_2011->(rlock()) .and. ucetsys->(rlock()) .and. prijatpl->(flock()) .and. uskutpl->(flock())
...
function FIN_dph_2011_pb(oxbp, nkeyCnt, nkeyNo, ncolor)
  local  charInf
