...
METHOD FIN_dphKohHd_INS:init(parent)
  ...
  * STATIC
  pa_oddilRadek := { {'A.1', 1 }, {'A.2', 1 }, {'A.3', 1 }, {'A.4', 1 }, {'A.5', 1 }, ;
                     {'B.1', 1 }, {'B.2', 1 }, {'B.3', 1 }                            }
...
static function dphKoh_rvs( dvystFak )
  ...
  *
  ...
  local  cdanDoklad := upper( if((hd_file)->(fieldPos('ndanDoklad')) = 0, allTrim(str((hd_file)->ndoklad,10)), (hd_file)->cdanDoklad ))
  ...
  for x := 1 to len(pa) step 1
        ...
        do while .not. (it_file)->( eof())
          if vykdph_i->nradek_dph = (it_file)->nradVykDph
            ...
            if dphKoh_iW->( dbseek( mky +strZero( (it_file)->ntypPreDan,3) ))
              ...
              dphKoh_iW->cdic       := (hd_file)->cdic
              dphKoh_iW->ndoklad    := (hd_file)->ndoklad
    ...
    case ( coddilKohl = 'A.2' )
       ...
       dphKoh_iW->nsazDan_1  += (hd_file)->nzaklDan_1

       dphKoh_iW->nzakldan_2 += (hd_file)->nzakldan_2
       dphKoh_iW->nsazDan_2  += (hd_file)->nzaklDan_2

       dphKoh_iW->nzakldan_3 += (hd_file)->nzakldan_3
       dphKoh_iW->nsazDan_3  += (hd_file)->nzaklDan_3
      ...
      if .not. empty(it_file)
        ...
        do while .not. (it_file)->( eof())
          if vykdph_i->nradek_dph = (it_file)->nradVykDph
            ...
            if dphKoh_iW->( dbseek( cky +strZero( (it_file)->ntypPreDan,3) ) )
...
static function aaddRec_and_doplnFirmu(coddilKohl)
  local  npos, nradek := 1
