...
function FIN_NAK_fakturovat_z_bc(col,head,cfile_hd)
  local  finTyp := if((fakPriHd->nFINTYP > 2 .or. fakPriHd->nFINTYP = 6), 2, 1)
  local  cel, uhr, par, parZal := 0
  *
  local  val := 0
  ...
  do case
  case( head = 1 )                                             // nabídka seznamu zálohových faktur

    default cfile_hd to 'fakPriHd'

    istuz_Zuc := Equal( zaklMena, fakPriHDw ->czkratMenZ)
    istuz_Zal := Equal( zaklMena, (cfile_Hd)->czkratMenZ )

    do case
    case( col = 1 )     // H - h -
      val := if( (cfile_Hd)->ncenZakCel = (cfile_Hd)->nuhrCelFak, H_big, ;
              if( (cfile_Hd)->nuhrCelFak = 0, 0, H_low))

    case( col = 2 )     // P - p -
      val := if( (cfile_Hd)->nuhrCelFak = (cfile_Hd)->nparZalFak, P_big, ;
              if( (cfile_Hd)->nparZalFak = 0, 0, P_low))

    case( col = 6 )     // záloha
      val := (cfile_Hd)->ncenZahCel

    case( col = 7 )     // úhrada
      val := (cfile_Hd)->nuhrCelFaz

    case( col = 8 )    // k uplatnìní V XXX
      fakprii_w->( dbseek( fakprii_w->ncisZalFak,, 'FAKPRIIT_2'))
*      parZal := abs(fakprii_W->ncecPrKdz)
      parZal := fakPrii_w->nparZahFak * if( fakPrii_w->_delrec = '9', +1, 0 )

      * shodné meny zuctovací i zálohové faktury
      if fakprihdW->czkratMenZ = (cfile_Hd)->czkratMenZ
        cel := (cfile_Hd)->ncenZahCel
        uhr := (cfile_Hd)->nuhrCelFaz
        par := (cfile_Hd)->nparZahFak

      else
      * ruzné meny zuctovací a zálohové faktury
        do case
        case istuz_Zuc .and. .not. istuz_Zal            // zuctovací v tuzemské mene   - zálohová v zahranicní
          kurzit->( dbseek( upper( fakPriHD->czkratMenZ),,'KURZIT9'))

          nkoe := kurzit->nkurzStred/ kurzit->nmnozPrep
          cel  := (cfile_Hd)->ncenZahCel * nkoe
          uhr  := (cfile_Hd)->nuhrCelFaz * nkoe
          par  := (cfile_Hd)->nparZahFak * nkoe

        case .not. istuz_Zuc .and. istuz_Zal            // zuctovací v zahranicní mene - zálohová v tuzemské
          nkoe := fakpriHDw->nmnozPrep / fakPriHDw->nkurZahMen
          cel  := (cfile_Hd)->ncenZahCel * nkoe
          uhr  := (cfile_Hd)->nuhrCelFaz * nkoe
          par  := (cfile_Hd)->nparZahFak * nkoe

        case .not. istuz_Zuc .and. .not. istuz_Zal      // zuctovací v zahranicní mene - zálohová v zahranicní
          kurzit->( dbseek( upper( (cfile_Hd)->czkratMenZ),,'KURZIT9'))

          nkoe := (kurzit->nkurzStred/ kurzit->nmnozPrep) / (fakPriHDw->nkurZahMen/ fakpriHDw->nmnozPrep)
          cel  := (cfile_Hd)->ncenZahCel * nkoe
          uhr  := (cfile_Hd)->nuhrCelFaz * nkoe
          par  := (cfile_Hd)->nparZahFak * nkoe
        endcase
  ...
  case head = 2
    do case
    case(col = 7)
      val := DBGetVal('FAKPRIITzw->' +if(fintyp = 1, 'nCEJPRKBZ', 'nCEJPRKBZ'))
    case(col = 8)
      val := DBGetVal('FAKPRIITzw->' +if(fintyp = 1, 'nCECPRKDZ', 'nCENZAHCEL'))
    endcase
...
* static function FIN_NAK_fakturovat_z_csym_in()
*  return fakprii_w->( dbseek( fakprii_w->ncisZalFak,, 'FAKPRIIT_2'))



*
**
class FIN_NAK_fakturovat_z_vld
exported:
  method init, fakturovat_z_sel, favst_padz

hidden:
  var    dm, df, udcp, curr_var, iz_file
  method takeValue, favst_mnoD, favst_dph, favst_vyz, favst_czns, favst_rv
  method            favst_parz
  method            favst_pen
...
method FIN_NAK_fakturovat_z_vld:init(parent)
return self

*
**   fin_fakprihd_it_in/nak_dodldtPhd_in         - postValidate
***  cenzboz, dodlstPit, objvysit, fakvyshd      - fakpriit
***  cenzboz, objvysit                           - dodlstPit - postAppend povolí/zakáže editaci
method FIN_NAK_fakturovat_z_vld:fakturovat_z_sel(drgDialog)
  ...
  *
  if IsObject(odrg)
    ...
    isfav                := (Lower(drgParse(odrg:name,'-')) = 'fakpriitw')
    value                := drgDialog:lastXbpInFocus:value
    it_file              := Lower(drgParse(odrg:name,'-'))
    ::iz_file := iz_file := if(items = 'ncislodl'   , 'dodlstpit' , ;
                            if(items = 'ccislobint' , 'objvysit'  , ;
                            if(items = 'nciszalfak' , 'fakprihd'  , 'cenzboz' )))

    do case
    case(items = 'ncislodl'  )                   // - dodlsptit -
    ...
    case(items = 'ccislobint')                   // - objvysit -
      iz_pos := 4
      cf     := "ncisfirmy = %%" +if( .not. empty(value)," .and. ccislobint = '%%'", '')
      filter := format(cf, if(empty(value),{(hd_file)->ncisFirmy},{(hd_file)->ncisFirmy,value}))

    case(items = 'nciszalfak')                   // - fakprihd & fakpriit -
      fordRec({'fakprihd'})

      filter := "ncisFirmy = %% .and. nfinTyp = %% .and. (nuhrCelFak <> nparZalFak)"
      finTyp := (hd_file)->nfinTyp
      parTyp := if(finTyp = 4 .or. finTyp = 6, '5', '3')
      iz_pos := 5
      cisFir := strzero((hd_file)->ncisfirmy,5)
      *
      ** p.Kozákové v STS to tady 2x padlo, zaboha nemùžu pøijít na to co dìlala, tak to zasichrujeme
      if empty(::udcp:m_sel_filter)
        ::udcp:m_sel_filter := "(" +format( filter, { (hd_file)->ncisFirmy, val(parTyp) } ) +")"
      endif
...
// JS      showDlg := (items = 'nciszalfak') .or. .t. // .or. (iz_file)->(Ads_GetRecordCount()) > 1)
      showDlg := (items = 'nciszalfak') .or. ((iz_file)->( ads_getKeyCount(1)) > 1)
    endif

    if showDlg
      odialog := drgDialog():new('FIN_NAK_fakturovat_z_SEL',drgDialog)
      odialog:create(,,.T.)
    ...
    if .not. showDlg .or. (nexit != drgEVENT_QUIT)
      ...
      * ukládáme vybrané položky v cyklu
      if showDlg .and. odialog:udcp:sp_saved .and. isMethod(::udcp, 'sp_overPostLastField')
        ...
        ::takeValue(it_file,iz_file,iz_pos)
        if( isMethod( self, 'after_fakturovat_z_sel'), ::after_fakturovat_z_sel(), nil )
...
//        if(isMethod(::udcp, 'showGets'), ::udcp:showGets(,.t.), nil)
    ...
    if(items = 'nciszalfak')  ; fordRec()
                                fakvyshd->(ads_clearAof(), dbclearScope())
...
method FIN_NAK_fakturovat_z_vld:takeValue(it_file,iz_file,iz_pos)
  local  x, pos, value, items, mname, par, iz_recs := (iz_file)->(recno()), nullDph
  local  mnozReODB := 0
* fakvys/vnpitw,     dodlstpit,      vyrzakit,        objitem,        fakprihd,       cenzboz  ,    fakvyshd_p
*
  local  pa := { ;
{      'cskp',             '',         'cskp',             '',              '',              '',             '' }, ;
...
{ 'ncejprzbz',    'ncejprzbz',      'ncenamj',    'ncenazakl', ':favst_parz/4',     'ncenapzbo',              0 }, ;
...
{     'cucet',        'cucet',   'cucettrzeb',             '',     'cucet_Uct', ':favst_czns/0',             '' }, ;
{  'cnazpol1',     'cnazpol1',     'cnazpol1',             '',              '', ':favst_czns/1',             '' }, ;
{  'cnazpol2',     'cnazpol2',     'cnazpol2',             '',              '', ':favst_czns/2',             '' }, ;
{  'cnazpol3',     'cnazpol3',     'cnazpol3',             '',              '', ':favst_czns/3',             '' }, ;
{  'cnazpol4',     'cnazpol4',     'cnazpol4',             '',              '', ':favst_czns/4',             '' }, ;
{  'cnazpol5',     'cnazpol5',     'cnazpol5',             '',              '', ':favst_czns/5',             '' }, ;
{  'cnazpol6',     'cnazpol6',     'cnazpol6',             '',              '', ':favst_czns/6',             '' }, ;
...
{   'nobjemj',      'nobjemj',       'nobjem',      'nobjemj',               0,        'nobjem',              0 }  }
  ...
  *
  ** vazba na daòové doklady zùètování záloh pøijatých
  ** nisParZal 1 - není DD / 2 - je DD
  if iz_file = 'fakprihd'
    ...
    nullDph   := if(fakprihdw->nfinTyp > 2 .or. fakprihdw->nfinTyp = 6, 14, 4)
...
method FIN_NAK_fakturovat_z_vld:favst_mnoD(par)
  local mnozDzbo := 0
...
*
method FIN_NAK_fakturovat_z_vld:favst_dph(par)
  local  aiz     := {'dodlstpit','vyrzakit','objitem','','cenzboz'}
...
method FIN_NAK_fakturovat_z_vld:favst_vyz(par)
  local  retVal
...
method FIN_NAK_fakturovat_z_vld:favst_czns(par)
...
method FIN_NAK_fakturovat_z_vld:favst_rv(par)
...
method FIN_NAK_fakturovat_z_vld:favst_padz(ncisZalFak)
  local  cky := upper(fakprihdw->cdenik) +strZero(::dm:get('fakpriitw->ncisZalFak'), 10)
  local  pky
  local  lis_zal := .f., lnewRec

  if isNumber(ncisZalFak)
    cky := upper(fakprihdw->cdenik) +strZero( ncisZalFak, 10)
  endif
...
method FIN_NAK_fakturovat_z_vld:favst_parz(par)
  do case
  case(par = 1)
    return 'FAK_' +alltrim(str(fakPriHd->ncisFak)) +' odpoèet zálohy'
  case(par = 2)
    return -1
  case(par = 3)
    return 'x'
  case(par = 4)
    return FIN_NAK_fakturovat_z_bc(8,1)
  case(par = 5)
...
method FIN_NAK_fakturovat_z_vld:favst_pen(par)
...
*
** CLASS FIN_NAK_fakturovat_z_SEL *************************************************
** základní tøída pro kontrolu/nabídku fakvysitw/fakvnpitw
function FIN_NAK_fakturovat_z_sel_is(ocol_is)
  local xval := ocol_is:getData()
return (xval = 6001)


CLASS FIN_NAK_fakturovat_z_SEL FROM drgUsrClass
EXPORTED:
  ...
  *
  ** DODLSTIPT stav / ceníková položka / k dispozici na skladu / _?_ k fakturaci
...
* FAKVYSHD - H_razeno/ P_árováno/ D_ny pøekroèené splatnosti
  ...
  ** 3 - u položky dokladu kusovníkový rozpad položka dokladu - cenzboz - kusov
  inline method FIN_NAK_cenzboz_ses(drgDialog)
    local  odialog, nexit := drgEVENT_QUIT

    DRGDIALOG FORM 'FIN_NAK_cenzboz_SEST' PARENT ::drgDialog MODAL DESTROY EXITSTATE nExit
  return


  inline method FAZ_itemMarked()
    local  filter := format("ncisfak = %%",{fakprihd->ncisFak})

    vykdph_pw->(dbclearfilter(), dbgotop())

    ::m_udcp:favst_padz( fakprihd->ncisFak )
...
HIDDEN:
  ...
  **  pøi zobrazení seznamu je nataven pohled na nevyfakturované
...
METHOD FIN_NAK_fakturovat_z_SEL:init(parent)
  local items
  *
  ...
  ::in_file := if(items = 'ncislodl'   , 'dodlstit'  , ;
               ...
               if(items = 'nciszalfak' , 'fakprihd'  , ;
               if(items = 'ncispenfak' , 'fakvyshd_p', 'cenzboz' )))))

  ::m_filter := (::in_file)->(ads_getAof())
  ::m_udcp   := parent:parent:udcp

// JS  ::popUp    := 'Kompletní seznam ,Nevyfakturované, Vyfakturované, Èásteènì fakturované'
...
METHOD FIN_NAK_fakturovat_z_SEL:getForm()
  ...
  headTitle := if(::in_file = 'dodlstit'  , 'dodacích listù'         , ;
               ...
               if(::in_file = 'fakprihd'  , 'zálohových faktur'      , ;
  ...
  * Pøevzít z Výrobních zakázek    ->vyrzakit
    ...
    DRGDBROWSE INTO drgFC FPOS 0,1.1 SIZE 110,13 FILE 'VYRZAKIT'     ;
      FIELDS 'M->vyrzakit_is::2.7::2,'                             + ;
             ...
             'FIN_NAK_fakturovat_z_bc(5;3):množKFak,'                  + ;
             'FIN_NAK_fakturovat_z_bc(6;3):H:2.6::2,'                  + ;
             'nRozm_del:délka,'                                    + ;
             'nRozm_sir:šíøka,'                                    + ;
             'nRozm_vys:výška,'                                    + ;
             'cRozm_MJ:mj'                                           ;
  ...
  * Pøevzít z Objednávek pøijatých ->objitem
  ...
  case ::in_file = 'fakprihd'
  * Fakturovat ze Zálohových faktur   ->fakprihd
    ccolumn := 'FIN_NAK_fakturovat_z_bc(8;1):k uplatnìní v ' +zkratMenz +':13'

    DRGDBROWSE INTO drgFC FPOS 0,0.1 SIZE 110, 9 FILE 'FAKPRIHD'     ;
      FIELDS 'FIN_NAK_fakturovat_z_bc(1;1):H:2.6::2,'              + ;
             'FIN_NAK_fakturovat_z_bc(2;1):P:2.6::2,'              + ;
             'nCISFAK:èísZálFak,'                                  + ;
             'cVARSYM:varSymbol,'                                  + ;
             'nCISFIRMY:firma,'                                    + ;
             'cNAZEV:název firmy:16,'                              + ;
             'dVYSTFAK:datVyst,'                                   + ;
             'FIN_NAK_fakturovat_z_bc(6;1):záloha:13,'             + ;
             'FIN_NAK_fakturovat_z_bc(7;1):úhrada:13,'             + ;
             'FIN_NAK_fakturovat_z_bc(9;1):v:4,'                   + ;
             'FIN_NAK_fakturovat_z_bc(8;1):k uplatnìní v ' +zkratMenz +':14' ;
      SCROLL 'yy' CURSORMODE 3 PP 7 POPUPMENU 'y' ITEMMARKED 'FAZ_itemMarked'

//  'FIN_NAK_fakturovat_z_bc(8;1):k uplatnìní:13,'        + ;

    DRGDBROWSE INTO drgFC FPOS 0,9.1 SIZE 109,6 FILE 'vykdph_Pw'     ;
      FIELDS 'M->preDanPov::2.7::2,'                               + ;
             'nradek_Dph:øv:5,'                                    + ;
             'M->cradek_Dph:název øádku výkazu dph:30,'            + ;
...
METHOD FIN_NAK_fakturovat_z_SEL:drgDialogInit(drgDialog)
  local  apos, asize
...
METHOD FIN_NAK_fakturovat_z_SEL:drgDialogStart(drgDialog)
  ...
  *
  ...
  local  ctag_old, ckey_old, lok_old := .f.
  local  ocol, chead := 'k uplatnìní v ' +(::m_udcp:hd_file)->czkratMenZ


  ::d_bro := d_bro

  pa_tagKey := drgScrPos:getPos_forSel('FIN_nak_fakturovat_z_SEL', drgDialog, ::in_file)
  ctag_old  := pa_tagKey[1]
  ckey_old  := pa_tagKey[2]
  *
  ** u párovaných záloh musíme pøehodit záhlaví sloupce
  if ::in_file = 'fakprihd'
    begin sequence
      for x := 1 to d_Bro:oxbp:colCount step 1
        ocol := d_Bro:oxbp:getColumn(x)
        if 'fin_nak_fakturovat_z_bc(8;1):k uplatnìní v ' $ lower(ocol:frmColum)
  ...
  if isobject(::drgVar)
    ...
    if .not. empty(value := ::drgVar:oVar:value)
      ...
      case (::in_file = 'fakprihd'  )  ;  ctag := 'FPRIHD1'   // n
...
method FIN_NAK_fakturovat_z_sel:drgDialogEnd(drgDialog)
 (::in_file)->(dbclearFilter())
return self


method FIN_NAK_fakturovat_z_sel:mark_doklad(drgDialog)
...
method FIN_NAK_fakturovat_z_sel:save_marked()
  ::sp_saved := .t.
...
//// tohle pøijde ven
method FIN_NAK_fakturovat_z_SEL:itemMarked()
  local  it_ky   := strZero(fakvyshd->ncisFak,10), uc_ky
  local  nparZal := FIN_NAK_fakturovat_z_bc(8,1)
  *
  ...
  *
  ...
  if nparZal > 0
    if(nosvODdan +nzaklDan_1 +nsazDan_1 +nzaklDan_2 +nsazDan_2) <> 0
      ...
      if( nosvOdDan <> 0, FIN_NAK_fakturovat_z_dan(nosvOdDan,'(osv)'), nil)

      if nzaklDan_1 <> 0
        FIN_NAK_fakturovat_z_dan(nzaklDan_1,'(ssd)')
      ...
      if nzaklDan_2 <> 0
        FIN_NAK_fakturovat_z_dan(nzaklDan_2,'(zsd)')
      ...
      FIN_NAK_fakturovat_z_dan(nparZal,'')
    endif
  endif
...
return self
*
static function FIN_NAK_fakturovat_z_dan(nparZal,ctx)
...
**
METHOD FIN_NAK_fakturovat_z_SEL:createContext()
...
METHOD FIN_NAK_fakturovat_z_SEL:fromContext(aorder,nmenu)
...
/*
  otherwise
    ...
    case(aorder = 2)                               // Nevyfakturované
      do case
      case(::in_file = 'dodlstit')  ;  filter += " (nstav_fakt = 0 .or. nstav_fakt = 1)"
      case(::in_file = 'vyrzakit')  ;  filter += " (nstavfakt  = 0 .or. nstavfakt  = 1)"
      case(::in_file = 'objitem' )  ;  filter += " (nstav_fakt = 0 .or. nstav_fakt = 1)"
      endcase

    case(aorder = 3)                              // Vyfakturované
      do case
      case(::in_file = 'dodlstit')  ;  filter += " nstav_fakt = 2"
      case(::in_file = 'vyrzakit')  ;  filter += " nstavfakt  = 2"
      case(::in_file = 'objitem' )  ;  filter += " nstav_fakt = 2"
      endcase

    case(aorder = 4)                              // Èásteènì fakturované
      do case
      case(::in_file = 'dodlstit')  ;  filter += " nstav_fakt = 1"
      case(::in_file = 'vyrzakit')  ;  filter += " nstavfakt  = 1"
      case(::in_file = 'objitem')   ;  filter += " nstav_fakt = 1"
      endcase
    endcase
...
*/
  ...
  if(aorder = 2 .or. aorder = 4)
    ...
    ** vyjmeme záznamy, kde množství pro pøevzetí je --> 0
    ...
    if( len( pa_exclude ) <> 0, ;
      (in_file)->(Ads_customizeAOF(pa_exclude, 2)), nil )


//    (::in_file) ->(dbeval( { || if( ocol_is:getData() <> 6001, aadd(pa, (in_file)->(recNo())), nil) }))

//    if( len(pa) <> 0, (::in_file)->(Ads_customizeAOF(pa, 2)), nil )
  endif

  if( .not. (::in_file)->( ads_isRecordInAOF(recNo)), (::in_file) ->(dbgotop()), nil )
...
CLASS FIN_NAK_cenzboz_SEST FROM drgUsrClass, fin_finance_in
EXPORTED:
...
METHOD FIN_NAK_cenzboz_sest:init(parent)
  local  hd_file

  if lower(parent:parent:formName) = 'FIN_NAK_fakturovat_z_sel'
...
METHOD FIN_NAK_cenzboz_sest:getForm()
  local  oDrg, drgFC
...
method FIN_NAK_cenzboz_sest:drgDialogInit(drgDialog)
...
METHOD FIN_NAK_cenzboz_sest:drgDialogStart(drgDialog)
...
METHOD FIN_NAK_cenzboz_sest:destroy()
RETURN