...
exported:
  ...
  *
  inline method overPostAppend()
    local filter := ::m_filter +" .and. (nuhrCelFak <> nparZalFak)"
    local ok, cc := ''

    parprzi_w->(dbgotop(), ;
                dbeval( { || cc += "ncisFak <> " +str( parprzi_w->ncisZalFak) +" .or. " } , ;
                        { || parprzi_w->_delRec <> '9' }  ))

    if( .not. empty(cc), filter += " .and. (" +left( cc, len(cc) -6) +")", nil )

    fakprih_ow->(ads_setAof(filter), dbgoTop())
...
method FIN_parprzal:parprzal_cvarsym_vld(drgDialog)
  ...
  *
  ...
  if IsObject(drgDialog) .or. drgVar:changed()
    ...
    if( empty( ::m_sel_filter), ::overPostAppend(), nil )
...
*
**  CLASS for FIN_parprzal_cvarsym_sel
*
...
method FIN_parprzal_cvarsym_sel:init(parent)
  ...
  *
  ...
  ::parent:pushArea()
  ::m_filter := ::parent:UDCP:m_filter
  ::popState := 1

  nEvent := LastAppEvent(@mp1,@mp2,@oXbp)
  if( IsOBJECT(oXbp:cargo), ::drgGet := oXbp:cargo, NIL )

  ::filter := ::parent:UDCP:m_sel_filter
